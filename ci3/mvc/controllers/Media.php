...
*/
	...
	public function index() {
		...
		$allUserTypes = $this->usertype_m->get_order_by_usertype_with_or(array('schoolID' => $schoolID));
		...
		$share_table = $this->media_share_m->get_order_by_media_share(array('schoolID' => $schoolID));

		if($this->data['usertypeID'] == 1) {
			$this->data['folders'] = $this->media_category_m->get_order_by_mcategory(array('schoolID' => $schoolID));
			$this->data['files']   = $this->media_m->get_order_by_media(array('mcategoryID'=>0,'schoolID'=>$schoolID));
		} else {
			$this->data['folders'] = $this->media_category_m->get_order_by_mcategory(array('userID'=> $this->data['userID'], 'usertypeID'=>$this->data['usertypeID'], 'schoolID'=>$schoolID));
			$this->data['files']   = $this->media_m->get_order_by_media(array('userID'=> $this->data['userID'], 'usertypeID'=>$this->data['usertypeID'], 'mcategoryID'=>0, 'schoolID'=>$schoolID));
		}
		...
		foreach ($share_table as $key => $item) {
			if ($item->public) {
				...
				if ($this->data['usertypeID'] == 3) {
					$student = $this->studentrelation_m->get_single_student(array('srstudentID'=>$this->data['userID'],'srschoolyearID'=>$schoolyearID,'srschoolID'=>$schoolID));
	...
	public function view() {
		...
		$folder_info = $this->media_category_m->get_single_media_category(array('mcategoryID' => $folderID, 'schoolID' => $schoolID));
		$this->data['f'] = $this->media_category_m->get_single_media_category(array('mcategoryID' => $folderID, 'schoolID' => $schoolID));
		if ((int)$folderID) {
			$this->data['files'] = $this->media_m->get_order_by_media(array("mcategoryID"=>$folderID,"schoolID"=>$schoolID));
		...
		if ($this->input->post('share_with') == "0") {
			...
			if (strpos($media_info,'folder') !== false) {
			    ...
			    $array['item_id'] = $folderID['1'];
				  $is_shared_media = $this->media_share_m->get_single(array('file_or_folder'=> 1,'item_id' => $array['item_id'],'schoolID' => $schoolID));
			} else {
				$array['file_or_folder'] = 0;
				$array['item_id'] = $this->input->post('media_info');
				$is_shared_media = $this->media_share_m->get_single(array('file_or_folder'=> 0,'item_id' => $array['item_id'],'schoolID' => $schoolID));
			}
	...
	public function deletef() {
		if(permissionChecker('media_delete')) {
			...
			if($id) {
				$all_files = $this->media_m->get_order_by_media(array("mcategoryID"=>$id,"schoolID"=>$schoolID));
				...
				$mediaCategory = $this->media_category_m->get_single_media_category(array('mcategoryID' => $id, 'schoolID' => $schoolID));
	...
	public function delete() {
		...
		$schoolID = $this->session->userdata('schoolID');
		if($id) {
			$file = $this->media_m->get_single_media(array('mediaID' => $id, 'schoolID' => $schoolID));
			if(($usertypeID == $file->usertypeID && $userID == $file->userID) || ($usertypeID == 1)) {
				$path = "uploads/media/".$file->file_name;
				if(config_item('demo') == FALSE) {
					if (unlink($path)) {
	...
	function classcall() {
		$schoolID = $this->session->userdata('schoolID');
		$allclass = $this->classes_m->get_order_by_classes(array('schoolID' => $schoolID));
