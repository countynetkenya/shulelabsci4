...
*/
	...
	protected function rules_smsleopard() {
		$rules = array(
			array(
				'field' => 'smsleopard_senderID',
				'label' => $this->lang->line("smssettings_senderID"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'smsleopard_accountID',
				'label' => $this->lang->line("smssettings_accountID"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'smsleopard_secret',
				'label' => $this->lang->line("smssettings_secret"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			)
		);
		return $rules;
	...
	protected function rules_bongasms() {
		$rules = array(
			array(
				'field' => 'bongasms_senderID',
				'label' => $this->lang->line("smssettings_senderID"),
				'rules' => 'trim|xss_clean|max_length[255]'
			),
			array(
				'field' => 'bongasms_api_clientID',
				'label' => $this->lang->line("smssettings_clientID"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'bongasms_api_secret',
				'label' => $this->lang->line("smssettings_secret"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'bongasms_api_key',
				'label' => $this->lang->line("smssettings_api_key"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			)
		);
	...
	public function unique_field($field) {
        if($this->input->post('type') == 'clickatell') {
            ...
            return TRUE;
        } elseif($this->input->post('type') == 'smsleopard') {
            if(!empty($this->input->post('smsleopard_senderID')) || !empty($this->input->post('smsleopard_accountID')) || !empty($this->input->post('smsleopard_secret'))) {

            	if($this->input->post('smsleopard_accountID') == $field) {
            		if($this->input->post('smsleopard_accountID') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            	...
            	if($this->input->post('smsleopard_secret') == $field) {
            		if($this->input->post('smsleopard_secret') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            	...
            	if($this->input->post('smsleopard_senderID') == $field) {
            		if($this->input->post('smsleopard_senderID') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            ...
            return TRUE;
        } elseif($this->input->post('type') == 'bongasms') {
            if(!empty($this->input->post('bongasms_api_clientID')) || !empty($this->input->post('bongasms_api_key')) || !empty($this->input->post('bongasms_api_secret'))) {

            	if($this->input->post('bongasms_clientID') == $field) {
            		if($this->input->post('bongasms_clientID') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            	...
            	if($this->input->post('bongasms_api_secret') == $field) {
            		if($this->input->post('bongasms_api_secret') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            	...
            	if($this->input->post('bongasms_api_key') == $field) {
            		if($this->input->post('bongasms_api_key') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
	...
	public function index() {
		...
		$bongasms_bind = array();
		$get_bongasms = $this->smssettings_m->get_order_by_bongasms(array('types' => 'bongasms', 'schoolID' => $schoolID));
		foreach ($get_bongasms as $key => $value) {
			$bongasms_bind[$value->field_names] = $value->field_values;
		}
		$this->data['set_bongasms'] = $bongasms_bind;

		$smsleopard_bind = array();
		$get_smsleopards = $this->smssettings_m->get_order_by_smsleopard(array('types' => 'smsleopard', 'schoolID' => $schoolID));
		foreach ($get_smsleopards as $key => $get_smsleopard) {
			$smsleopard_bind[$get_smsleopard->field_names] = $get_smsleopard->field_values;
		}
		$this->data['set_smsleopard'] = $smsleopard_bind;
    ...
    $msg91_bind = array();
		...
		if($_POST) {
			...
			if($type == 'smsleopard') {
				$this->data['smsleopard'] = 1;
				$this->data['clickatell'] = 0;
				$this->data['twilio'] = 0;
				$this->data['bulk'] = 0;
        $this->data['msg91'] = 0;

				$rules = $this->rules_smsleopard();
				$this->form_validation->set_rules($rules);
				if ($this->form_validation->run() == FALSE) {
					$this->data["subview"] = "smssettings/index";
					$this->load->view('_layout_main', $this->data);
				} else {
					$senderID = $this->input->post('smsleopard_senderID');
					$accountID = $this->input->post('smsleopard_accountID');
					$secret = $this->input->post('smsleopard_secret');
					...
					$this->smssettings_m->update_smsleopard($array);
					$this->data["subview"] = "smssettings/index";
					$this->load->view('_layout_main', $this->data);
				}
			} elseif($type == 'bongasms') {
				$this->data['bongasms'] = 1;
				$this->data['smsleopard'] = 0;
				$this->data['clickatell'] = 0;
				$this->data['twilio'] = 0;
				$this->data['bulk'] = 0;
        $this->data['msg91'] = 0;

				$rules = $this->rules_bongasms();
				$this->form_validation->set_rules($rules);
				if ($this->form_validation->run() == FALSE) {
					$this->data["subview"] = "smssettings/index";
					$this->load->view('_layout_main', $this->data);
				} else {
					$senderID = $this->input->post('bongasms_senderID');
					$clientID = $this->input->post('bongasms_api_clientID');
					$key = $this->input->post('bongasms_api_key');
					$secret = $this->input->post('bongasms_api_secret');
					...
					$this->smssettings_m->update_bongasms($array);
					$this->data["subview"] = "smssettings/index";
					$this->load->view('_layout_main', $this->data);
				}
			} elseif($type == 'clickatell') {
				$this->data['smsleopard'] = 0;
				$this->data['bongasms'] = 0;
