...
class Promotion extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ((int) $classesID && (int) $schoolyearID) {
            ...
            $this->data['sections']     = pluck_multi_array($this->section_m->general_get_order_by_section(array('schoolID' => $schoolID)), 'obj', 'classesID');
            $this->data['subjects']     = $this->subject_m->general_get_order_by_subject(['classesID' => $classesID, 'schoolID' => $schoolID]);
            ...
            $this->data['schoolyears'] = $this->data['topbarschoolyears'];
            $this->data['classes']     = $this->classes_m->general_get_order_by_classes(['schoolID' => $schoolID]);
    ...
    public function add()
    {
        ...
        if ((int) $classID && (int) $schoolyearID) {
            $classes    = $this->classes_m->general_get_order_by_classes(array('classesID' => $classID, 'schoolID' => $schoolID));
            ...
            if (customCompute($classes) && customCompute($schoolyear)) {
                ...
                $this->data['schoolyearID'] = $schoolyearID;
                $this->data['sections']     = pluck($this->section_m->general_get_order_by_section(['classesID' => $classID, 'schoolID' => $schoolID]),
                    'obj', 'sectionID');

                $this->studentPromotionCalculation($classID, $schoolyearID, $schoolID);
    ...
    private function studentPromotionCalculation($classID, $schoolyearID, $schoolID)
    {
        ...
        $subjects     = pluck($this->subject_m->get_order_by_subject(['classesID' => $classID, 'schoolID' => $schoolID]), 'obj',
            'subjectID');
        ...
        foreach ($studentStatus as $studentID => $value) {
            ...
            if (isset($students[$studentID])) {
                $studentStatus[$studentID]['info'] = (object) [
                    ...
                    "schoolID"     => $students[$studentID]->schoolID,
                ];
            } else {
                $studentStatus[$studentID]['info'] = (object) [
                    "studentID"    => $studentID,
    ...
    public function summary()
    {
        ...
        if ((int) $studentID && (int) $classID && (int) $schoolyearID) {
            ...
            $checkClass      = $this->classes_m->general_get_single_classes(['classesID' => $classID, 'schoolID' => $schoolID]);
            $checkSchoolyear = $this->schoolyear_m->get_single_schoolyear(['schoolyearID' => $schoolyearID, 'schoolID' => $schoolID]);

            if (customCompute($checkStudent) && customCompute($checkClass) && customCompute($checkSchoolyear)) {
                $this->data['set'] = $classID;
                $this->basicInfo($checkStudent);
                $this->getMark($studentID, $classID, $schoolyearID, $schoolID);
                $this->studentPromotionCalculation($classID, $schoolyearID, $schoolID);
    ...
    private function getMark($studentID, $classesID, $schoolyearID, $schoolID)
    {
        if ((int) $studentID && (int) $classesID && (int) $schoolyearID) {
            ...
            if (customCompute($student) && customCompute($classes)) {
                ...
                $subjects           = $this->subject_m->general_get_order_by_subject(array('classesID' => $classesID, 'schoolID' => $school));
                $subjectArr         = [];
                $optionalsubjectArr = [];
    ...
    public function promotion_to_next_class()
    {
        ...
        $sections      = $this->section_m->general_get_order_by_section(["classesID" => isset($enroll) && $enroll ? $previousClasseID : $promotionClassID, "schoolID" => $schoolID]);
        ...
        if (customCompute($students) && customCompute($studentIDs) && customCompute($previousClasseID)) {
            ...
            foreach ($explodeStudents as $key => $studentID) {
                ...
                if (isset($students[$studentID])) {
                    ...
                    if ($setClassesID > 0) {
                        $classesRelation = $this->classes_m->general_get_single_classes(array('classesID' => $setClassesID, 'schoolID' => $schoolID));
                    ...
                    if ($promoteSectionID > 0) {
                        $sectionRelation = $this->section_m->general_get_single_section(array('sectionID' => $promoteSectionID, 'schoolID' => $schoolID));
