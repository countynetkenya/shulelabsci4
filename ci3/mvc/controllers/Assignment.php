...
*/
	...
	public function fileupload() {
		...
		if((int)$id) {
			...
			$assignment = $this->assignment_m->get_single_assignment(array('assignmentID' => $id, 'schoolID' => $schoolID));
		}
	...
	public function index() {
		...
		if($this->session->userdata('usertypeID') == 3) {
			...
			$this->data['opsubjects']  = pluck($this->subject_m->get_order_by_subject(['classesID'=> $classesID, 'type'=>0, 'schoolID' => $schoolID]), 'subjectID', 'subjectID');
			$this->data['student']     = $this->studentrelation_m->get_single_studentrelation(['srstudentID'=> $loginuserID, 'srschoolyearID'=> $schoolyearID, 'srschoolID' => $schoolID]);
		} else {
		...
		if((int)$id && (int)$url) {
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
				...
				if(isset($fetchClasses[$url])) {
					$this->data['assignment'] = $this->assignment_m->get_single_assignment(array('assignmentID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
	...
	public function view() {
		...
		if((int)$assignmentID && (int)($classesID)) {
			...
			if(isset($fetchClasses[$classesID])) {
				...
				$assignment = $this->assignment_m->get_single_assignment(array('assignmentID' => $assignmentID, 'classesID' => $classesID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
			...
			if((int)$id && (int)($url)) {
				...
				if(isset($fetchClasses[$url])) {
					if($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) {
						$assignment = $this->assignment_m->get_single_assignment(array('assignmentID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
							...
							if(strtotime($assignment->deadlinedate) >= strtotime(date('Y-m-d'))) {
								if($_POST) {
									...
									if ($this->form_validation->run() == FALSE) {
										...
										$assignmentanswer = $this->assignmentanswer_m->get_single_assignmentanswer(array('uploaderID' => $userID, 'uploadertypeID' => $usertypeID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID, 'assignmentID' => $id));
	...
	public function answerdownload() {
		...
		if((int)$id) {
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$schoolID = $this->session->userdata('schoolID');
			$assignmentanswer = $this->assignmentanswer_m->get_single_assignmentanswer(array('assignmentanswerID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
			if(customCompute($assignmentanswer)) {
