...
*/
	...
	public function index() {
		$this->data['classes'] = $this->classes_m->general_get_order_by_classes(array('schoolID' => $this->session->userdata('schoolID')));
		...
		if(permissionChecker('attendancereport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$attendances = '';
					$classes 		= $this->classes_m->general_get_single_classes(array('classesID'=>$classesID,'schoolID'=>$schoolID));
					$day = 'a'.(int)$date[0];
					$monthyear = $date[1].'-'.$date[2];

					if($sectionID == 0) {
						$students = pluck($this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $classesID , 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID)), 'obj', 'srstudentID');
						$sections = pluck($this->section_m->general_get_order_by_section(array('classesID' => $classesID, 'schoolID' => $schoolID)), 'obj', 'sectionID');

						if($subjectID) {
							$attendances = $this->subjectattendance_m->get_order_by_sub_attendance(array('classesID' => $classesID, 'subjectID' => $subjectID, 'monthyear' => $monthyear, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));

						} else {
							$attendances = $this->sattendance_m->get_order_by_attendance(array('classesID' => $classesID, 'monthyear' => $monthyear, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
						}
					} else {
						$students = pluck($this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $classesID, 'srsectionID' => $sectionID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID)), 'obj', 'srstudentID');
						$sections 	= pluck($this->section_m->general_get_order_by_section(array( 'classesID' => $classesID, 'sectionID' => $sectionID, 'schoolID' => $schoolID)), 'obj', 'sectionID');

						if($subjectID) {
							$attendances = $this->subjectattendance_m->get_order_by_sub_attendance(array('classesID' => $classesID, 'sectionID' => $sectionID, 'subjectID' => $subjectID, 'monthyear' => $monthyear, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
						} else {
							$attendances = $this->sattendance_m->get_order_by_attendance(array('classesID' => $classesID, 'sectionID' => $sectionID, 'monthyear' => $monthyear, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
						}
					}
	...
	public function pdf() {
		if(permissionChecker('attendancereport')) {
			...
			if($dateFlag) {
				$classes = $this->classes_m->general_get_single_classes(array('classesID'=>$classesID,'schoolID'=>$schoolID));
				$section = $this->section_m->general_get_single_section(array('sectionID'=>$sectionID,'schoolID'=>$schoolID));
	...
	public function getSection() {
		...
		if((int)$id) {
			$sections = $this->section_m->general_get_order_by_section(array('classesID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
