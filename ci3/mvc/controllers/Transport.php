...
*/
	...
	public function index() {
		$this->data['transports'] = $this->transport_m->get_order_by_transport(array('schoolID' => $this->session->userdata('schoolID')));
	...
	public function edit() {
		...
		if((int)$id) {
			$this->data['transport'] = $this->transport_m->get_single_transport(array('transportID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function delete() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$transport = $this->transport_m->get_single_transport(array('transportID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	function unique_route() {
		...
		$schoolID = $this->session->userdata('schoolID');
		if((int)$id) {
			$transport = $this->transport_m->get_order_by_transport(array("route" => $this->input->post("route"), "transportID !=" => $id, 'schoolID' => $schoolID));
			if(customCompute($transport)) {
			...
			$transport = $this->transport_m->get_order_by_transport(array("route" => $this->input->post("route"), 'schoolID' => $schoolID));
