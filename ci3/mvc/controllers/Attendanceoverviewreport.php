...
class Attendanceoverviewreport extends Admin_Controller {
	...
	public function getSubject() {
		$classesID = $this->input->post('classesID');
		if((int)$classesID) {
			$subjects = $this->subject_m->general_get_order_by_subject(array('classesID' => $classesID, 'schoolID' => $this->session->userdata('schoolID')));
			echo "<option value='0'>", $this->lang->line("attendanceoverviewreport_please_select"),"</option>";
	...
	public function getStudent() {
		...
		if((int)$usertype && (int)$classesID && (int)$sectionID) {
			echo "<option value='0'>".$this->lang->line("attendanceoverviewreport_please_select")."</option>";
			if($usertype == 1) {
				$students = $this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $classesID,'srsectionID'=>$sectionID,'srschoolyearID' => $schoolyearID,'srschoolID' => $schoolID));
	...
	private function userQueryArray($posts) {
		...
		if($posts['usertype'] == '1') {
			$schoolID = $this->session->userdata('schoolID');
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$userQueryArray = array('srschoolyearID' => $schoolyearID, 'srclassesID' => $posts['classesID'], 'srschoolID' => $schoolID);
				  ...
				  echo json_encode($retArray);
					...
					if($usertype == '1') {
						if($this->data["siteinfos"]->attendance == 'subject') {
							...
							$this->data['subjects'] = pluck($this->subject_m->get_order_by_subject(array('classesID' => $classesID, 'schoolID' => $schoolID)),'subject','subjectID');
						} else {
							$attendances =  pluck($this->sattendance_m->get_order_by_attendance($queryArray),'obj','studentID');
						}
						$this->data['leaveapplications'] = $this->leave_applications_date_list_by_user_and_schoolyear(1,$schoolyearID);
