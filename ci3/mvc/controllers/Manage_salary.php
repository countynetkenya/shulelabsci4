...
class Manage_salary extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['roles'] = $this->usertype_m->get_order_by_usertype_with_or(array("schoolID" => $schoolID));
        ...
        if($setrole == 1) {
            ...
            $this->data['managesalary'] = pluck($this->manage_salary_m->get_order_by_manage_salary(array('usertypeID' => 1, 'schoolID' => $schoolID)), 'userID');
         } elseif($setrole == 2) {
            $this->data['users'] = $this->teacher_m->get_order_by_teacher(array('schoolID' => $schoolID));
            $this->data['managesalary'] = pluck($this->manage_salary_m->get_order_by_manage_salary(array('usertypeID' => 2, 'schoolID' => $schoolID)), 'userID');
        } else {
            $this->data['users'] = $this->user_m->get_order_by_user(array('usertypeID' => $setrole, 'schoolID' => $schoolID));
            $this->data['managesalary'] = pluck($this->manage_salary_m->get_order_by_manage_salary(array('usertypeID' => $setrole, 'schoolID' => $schoolID)), 'userID');
        }
    ...
    public function add() {
        ...
        if((int)$userID && (int) $usertypeID) {
            ...
            if(customCompute($user)) {
                $manageSalary = $this->manage_salary_m->get_single_manage_salary(array('usertypeID' => $usertypeID, 'userID' => $userID, 'schoolID' => $schoolID));
                if(!customCompute($manageSalary)) {
                    ...
                    if($this->input->post('salary')) {
                        ...
                        if($salary == 1) {
                            $this->data['alltemplate'] = $this->salary_template_m->get_order_by_salary_template(array('schoolID' => $schoolID));
                        } elseif($salary == 2) {
                            $this->data['alltemplate'] = $this->hourly_template_m->get_order_by_hourly_template(array('schoolID' => $schoolID));
                        }
                    } else {
    ...
    public function delete() {
        ...
        if((int)$userID && (int)$usertypeID) {
            ...
            if(customCompute($user)) {
                $this->data['manage_salary'] = $this->manage_salary_m->get_single_manage_salary(array('usertypeID' => $usertypeID, 'userID' => $userID, 'schoolID' => $schoolID));
    ...
    public function templatecall() {
        ...
        if($salary == 1) {
            $salaryTemplates = $this->salary_template_m->get_order_by_salary_template(array('schoolID' => $schoolID));
            ...
            $salaryTemplates = $this->hourly_template_m->get_order_by_hourly_template(array('schoolID' => $schoolID));
            if(customCompute($salaryTemplates)) {
                echo '<option value="0">'.$this->lang->line('manage_salary_select_template').'</option>';
                foreach ($salaryTemplates as $salaryTemplateKey => $salaryTemplate) {
