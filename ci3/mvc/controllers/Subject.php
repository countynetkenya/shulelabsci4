...
*/
	...
	protected function rules() {
		$rules = array(
				...
				array(
					'field' => 'targetmark',
					'label' => $this->lang->line("subject_targetmark"),
					'rules' => 'trim|required|xss_clean|max_length[11]|numeric'
				),
				array(
					'field' => 'subject',
				...
				array(
					'field' => 'report_order',
					'label' => $this->lang->line("subject_report_order"),
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$array = array(
					...
					'targetmark' => $this->input->post('targetmark'),
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				$this->data['teachers'] = $this->teacher_m->general_get_order_by_teacher(array("schoolID" => $schoolID));
				$this->data['subject'] = $this->subject_m->general_get_single_subject(array('subjectID' => $id, 'classesID' => $url, 'schoolID' => $schoolID));
	...
	public function delete() {
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				$subject = $this->subject_m->general_get_single_subject(array('subjectID' => $id, 'classesID' => $url, 'schoolID' => $schoolID));
	...
	public function unique_subject() {
		...
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$subject = $this->subject_m->general_get_order_by_subject(array("subject" => $this->input->post("subject"), "subjectID !=" => $id, "classesID" => $this->input->post("classesID"), "schoolID" => $schoolID));
			if(customCompute($subject)) {
			...
			$subject = $this->subject_m->general_get_order_by_subject(array("subject" => $this->input->post("subject"), "classesID" => $this->input->post("classesID"), "subject_code" => $this->input->post("subject_code"), "schoolID" => $schoolID));
	...
	public function unique_subject_code() {
		...
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$subject = $this->subject_m->general_get_order_by_subject(array("subject_code" => $this->input->post("subject_code"), "subjectID !=" => $id, "schoolID" => $schoolID));
			if(customCompute($subject)) {
			...
			$subject = $this->subject_m->general_get_order_by_subject(array("subject_code" => $this->input->post("subject_code"), "schoolID" => $schoolID));
