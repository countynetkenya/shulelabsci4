...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			if(isset($fetchClass[$id])) {
				...
				$this->data['sections'] = $this->section_m->general_get_order_by_section(array('classesID' => $id, 'schoolID' => $schoolID));
			} else {
				$this->data['teacher'] = [];
				$this->data['sections'] = [];
			}
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			$this->data['teachers'] = $this->teacher_m->general_get_order_by_teacher(array("schoolID" => $schoolID));
			$this->data['classes'] = $this->classes_m->get_order_by_classes(array("schoolID" => $schoolID));
			$this->data['section'] = $this->section_m->general_get_single_section(array('sectionID' => $id, 'classesID' => $url, 'schoolID' => $schoolID));
			$fetchClass = pluck($this->data['classes'], 'classesID', 'classesID');
	...
	public function delete() {
		...
		if((int)$id && (int)$url) {
			$schoolID = $this->session->userdata('schoolID');
			$section = $this->section_m->get_single_section(array('sectionID' => $id, 'schoolID' => $schoolID));
			$classes = $this->classes_m->get_single_classes(array('classesID' => $url, 'schoolID' => $schoolID));
	...
	public function unique_section() {
		...
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$section = $this->section_m->general_get_order_by_section(array("classesID" => $this->input->post("classesID"), "section" => $this->input->post('section'), "sectionID !=" => $id, "schoolID" => $schoolID));
			...
			$section = $this->section_m->general_get_order_by_section(array("classesID" => $this->input->post("classesID"), "section" => $this->input->post('section'), "schoolID" => $schoolID));
