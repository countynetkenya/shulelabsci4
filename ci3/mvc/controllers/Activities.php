...
*/
    ...
    $this->data['activitiescategories'] = pluck($this->activitiescategory_m->get_order_by_activitiescategory(array('schoolID' => $schoolID)), 'obj', 'activitiescategoryID');
    $this->data['activities'] = $this->activities_m->get_order_by_activities(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
    $this->data['activitiesmedia'] = pluck_multi_array($this->activitiesmedia_m->get_order_by_activitiesmedia(array('schoolID' => $schoolID)), 'obj', 'activitiesID');
    $this->data['activitiescomments'] = pluck_multi_array($this->activitiescomment_m->get_order_by_activitiescomment(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'activitiesID');
    ...
    public function loadRecord(){
        ...
        $data = $this->activities_m->get_activities_pagination($this->input->post('limit'), $this->input->post('start'),$schoolyearID,$schoolID);
        $load_data = '';
        if($data->num_rows() > 0)
        {
	...
	public function add() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if((int)$categoryID) {
								$schoolID = $this->session->userdata('schoolID');
                $schoolyearID = $this->session->userdata('defaultschoolyearID');
                $this->data['activities_categories'] = $this->activitiescategory_m->get_order_by_activitiescategory(array('schoolID' => $schoolID));
                if($_POST) {
                    $rules = $this->rules();
                                ...
                                for($i = 0; $i < $filesCount; $i++){
                                    ...
                                    if($this->upload->do_upload('attach')){
                                        ...
                                        $uploadData[$i]['create_date'] = date("Y-m-d H:i:s");
																				$uploadData[$i]['schoolID'] = $schoolID;
                                    }
                                }
                                if(!empty($uploadData)){
	...
	public function delete() {
        ...
        $userID = $this->session->userdata('loginuserID');
    		...
    		if((int)$id) {
          $activities = $this->activities_m->get_single_activities(array('activitiesID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function delete_comment() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if((int)$id) {
                $comment = $this->activitiescomment_m->get_single_activitiescomment(array('activitiescommentID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
