...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			...
			$student = $this->studentrelation_m->get_single_student(array("srstudentID" => $studentID, 'srschoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
			...
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$parent = $this->parents_m->get_single_parents(array('parentsID' => $parentID, 'schoolID' => $schoolID));
			if(customCompute($parent)) {
				...
				if((int)$id) {
					$this->data['set'] = $id;
					$checkstudent = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
			...
			if($lID != "" || !empty($lID)) {
				...
				$this->data['issues'] = $this->issue_m->get_issue_with_books(array("lID" => $ulID, 'schoolID' => $schoolID));
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$schoolID = $this->session->userdata('schoolID');
			$this->data['books'] = $this->book_m->get_order_by_book(array('schoolID' => $schoolID));
			if($_POST) {
				$rules = $this->rules();
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$quantity = $this->book_m->get_single_book(array("bookID" => $this->input->post("book"), 'schoolID' => $schoolID));
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$this->data['books'] = $this->book_m->get_order_by_book(array('schoolID' => $schoolID));
			if((int)$id && $lID) {
				$this->data['issue'] = $this->issue_m->get_issue_with_books(array('issueID' => $id, 'schoolID' => $schoolID), TRUE);
				$dbGet_bookID = $this->data['issue']->bookID;
				$this->data['bookinfo'] = $this->book_m->get_single_book(array('bookID' => $dbGet_bookID, 'schoolID' => $schoolID));
	...
	public function view() {
		...
		if((int)$id) {
			$schoolID = $this->session->userdata('schoolID');
			$this->data['book'] = $this->issue_m->get_issue_with_books(array('issueID' => $id, 'schoolID' => $schoolID), TRUE);
		...
		if(permissionChecker('issue_view')) {
			if($_POST) {
			    ...
			    exit;
					...
					if((int)$id) {
						$this->data['book'] = $this->issue_m->get_issue_with_books(array('issueID' => $id, 'schoolID' => $this->session->userdata('schoolID')), TRUE);
						if($this->data['book']) {
							$lmember = $this->lmember_m->get_single_lmember(array('lID' => $this->data['book']->lID));
							if(customCompute($lmember)) {
								$this->data['lmember'] = $lmember;
	...
	public function returnbook() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if(permissionChecker('issue_add') && permissionChecker('issue_edit')) {
				if((int)$id && $lID) {
					$date = date("Y-m-d");
					$issue = $this->issue_m->get_single_issue(array('issueID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function unique_quantity() {
		...
		if($id) {
			if((int)$bookID) {
				$bookandauthor = $this->issue_m->get_single_issue(array("bookID" => $bookID, "issueID" => $id, 'schoolID' => $schoolID));
			...
			if((int)$bookID) {
				$bookandauthor = $this->book_m->get_single_book(array("bookID" => $bookID, 'schoolID' => $schoolID));
				if(customCompute($bookandauthor)) {
	...
	public function bookIDcall() {
		...
		if($bookID) {
			$bookinfo = $this->book_m->get_single_book(array('bookID' => $bookID, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function match_bookauthor() {
		...
		if((int)$bookID && $bookID != "") {
			$bookandauthor = $this->book_m->get_single_book(array("bookID" => $bookID, 'schoolID' => $this->session->userdata('schoolID')));
			if($bookandauthor) {
	...
	public function add_invoice() {
		...
		if(permissionChecker('issue_add') && permissionChecker('issue_edit')) {
			if($libraryID && $amount) {
				$schoolID = $this->session->userdata('schoolID');
				$librarymember = $this->issue_m->get_student_by_libraryID_with_studenallinfo(array('lmember.lID' => $libraryID, 'schoolID' => $schoolID));

				if(customCompute($librarymember)) {
					$feetype = $this->feetypes_m->get_single_feetypes(array('feetypes' => $this->lang->line('issue_bookfine'), 'schoolID' => $schoolID));

					if(!customCompute($feetype)) {
						$this->feetypes_m->insert_feetypes(array('feetypes' => $this->lang->line('issue_bookfine'), 'schoolID' => $schoolID, 'note' => "Don't delete it!"));
					}
