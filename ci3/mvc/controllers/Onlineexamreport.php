...
*/
	...
	public function index() {
		...
		$this->data['onlineexams'] 	= $this->online_exam_m->get_order_by_online_exam(array('schoolYearID' => $schoolyearID, 'schoolID' => $schoolID));
	...
	public function getSection() {
		$classesID = $this->input->post('classesID');
		if((int)$classesID) {
			$allSection = $this->section_m->general_get_order_by_section(array('classesID' => $classesID, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function getUserList() {
		...
		if($_POST) {
		    ...
		    exit;
				...
				$queryArray = [];
				$examArray = [];
				$examArray['schoolID'] = $schoolID;
				$queryArray = array('srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID);
				...
				$this->data['onlineexams'] = pluck($this->online_exam_m->get_order_by_online_exam(array('schoolYearID' => $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'onlineExamID');
				$this->data['students'] = pluck($this->studentrelation_m->general_get_order_by_student($queryArray), 'obj', 'srstudentID');
				$this->data['classs'] 	= pluck($this->classes_m->general_get_order_by_classes(array('schoolID' => $schoolID)), 'obj', 'classesID');
				$this->data['sections'] = pluck($this->section_m->general_get_order_by_section(array('schoolID' => $schoolID)), 'obj', 'sectionID');
				$this->data['subjects'] = pluck($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'subjectID');
	...
	public function result() {
		...
		if(permissionChecker('onlineexamreport')) {
			if((int)$onlineExamUserStatusID) {
				$schoolID = $this->session->userdata('schoolID');
				$onlineExamUserStatus = $this->online_exam_user_status_m->get_single_online_exam_user_status(array('onlineExamUserStatus' => $onlineExamUserStatusID, 'schoolID' => $schoolID));
				if(customCompute($onlineExamUserStatus)) {
					$schoolyearID = $this->session->userdata('defaultschoolyearID');
					$this->data['onlineExamUserStatus'] = $onlineExamUserStatus;
					$this->data['onlineexam'] = $this->online_exam_m->get_single_online_exam(array('onlineExamID' => $onlineExamUserStatus->onlineExamID, 'schoolID' => $schoolID));
					if((int)$this->data['onlineexam']->subjectID) {
						$this->data['subject'] = $this->subject_m->general_get_single_subject(array('subjectID' => $this->data['onlineexam']->subjectID, 'schoolID' => $schoolID));
					} else {
						$this->data['subject'] = [];
					}
					$this->data['rank'] = $this->ranking($onlineExamUserStatus->onlineExamID,$onlineExamUserStatusID, $onlineExamUserStatus->userID, $schoolID);
					$this->data['student'] = $this->studentrelation_m->general_get_single_student(array('srstudentID' => $onlineExamUserStatus->userID,'srschoolyearID'=>$schoolyearID,'srschoolID'=>$schoolID));

					$this->data['classes'] = pluck($this->classes_m->general_get_order_by_classes(array('schoolID' => $schoolID)), 'classes', 'classesID');
					$this->data['section'] = pluck($this->section_m->general_get_order_by_section(array('schoolID' => $schoolID)), 'section', 'sectionID');
	...
	private function ranking($onlineexamID, $onlineexamuserstatusID, $userID, $schoolID) {
		$onlineExamUserStatus = $this->online_exam_user_status_m->get_order_by_online_exam_user_status(array('onlineExamID' => $onlineexamID, 'schoolID' => $schoolID));
		$retArray = [];
		if(customCompute($onlineExamUserStatus)) {
