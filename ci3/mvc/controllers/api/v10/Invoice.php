...
class Invoice extends Api_Controller
{
    ...
    public function index_get()
    {
        ...
        if($usertypeID == 3) {
            ...
            if(customCompute($student)) {
                ...
                $this->retdata['grandtotalandpayment'] = $this->grandtotalandpaid($this->retdata['maininvoices'], $schoolyearID, $schoolID);
            ...
            $this->retdata['maininvoices'] = $this->maininvoice_m->get_maininvoice_with_studentrelation(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
            $this->retdata['grandtotalandpayment'] = $this->grandtotalandpaid($this->retdata['maininvoices'], $schoolyearID, $schoolID);
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/invoice/view/{maininvoiceID}",
       *     @OA\Parameter(
       *         name="maininvoiceID",
       *         description="",
       *         in = "path",
    ...
    public function view_get($id = null)
    {
        ...
        $usertypeID = $this->session->userdata("usertypeID");
        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        $this->retdata['feetypes'] = pluck($this->feetypes_m->get_order_by_feetypes(array('schoolID' => $schoolID)), 'feetypes', 'feetypesID');
        ...
        if($usertypeID == 3) {
            if((int)$id) {
                ...
                if(customCompute($getstudent)) {
                    $this->retdata['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_maininvoiceID(array('invoiceID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                    if(customCompute($this->retdata['maininvoice']) && ($this->retdata['maininvoice']->maininvoicestudentID == $getstudent->studentID)) {
                        $invoices = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $id, 'schoolID' => $schoolID));
                        ...
                        $this->retdata['grandtotalandpayment'] = $this->grandtotalandpaidsingle($this->retdata['maininvoice'], $schoolyearID, $schoolID, $this->retdata["maininvoice"]->maininvoicestudentID);
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/invoice/paymentlist/{maininvoiceID}",
       *     @OA\Parameter(
       *         name="maininvoiceID",
       *         description="",
       *         in = "path",
    ...
    public function paymentlist_get($maininvoiceID = null)
    {
        if(permissionChecker('invoice_view')) {
            ...
            if(!empty($maininvoiceID) && (int)$maininvoiceID && $maininvoiceID > 0) {
                $maininvoice = $this->maininvoice_m->get_single_maininvoice(array('maininvoiceID' => $maininvoiceID, 'maininvoiceschoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/invoice/viewpayment/{globalpaymentID}/{maininvoiceID}",
       *     @OA\Parameter(
    ...
    public function viewpayment_get($globalpaymentID = null, $maininvoiceID = null)
    {
        if(permissionChecker('invoice_view')) {
            ...
            if((int)$globalpaymentID && (int)$maininvoiceID) {
                $globalpayment = $this->globalpayment_m->get_single_globalpayment(array('globalpaymentID' => $globalpaymentID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                $maininvoice = $this->maininvoice_m->get_single_maininvoice(array('maininvoiceID' => $maininvoiceID, 'maininvoiceschoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                if(customCompute($maininvoice) && customCompute($globalpayment)) {
                    ...
                    if($f) {
                        ...
                        if(customCompute($studentrelation)) {
                            $this->retdata['feetypes'] = pluck($this->feetypes_m->get_order_by_feetypes(array('schoolID' => $schoolID)), 'feetypes', 'feetypesID');
                            $this->retdata['student'] = $this->student_m->get_single_student(array('studentID' => $globalpayment->studentID, 'schoolID' => $schoolID));
                            $this->retdata['invoices'] = pluck($this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $maininvoiceID, 'schoolID' => $schoolID)), 'obj', 'invoiceID');

                            $this->payment_m->order_payment('paymentID', 'asc');
                            $this->retdata['payments'] = $this->payment_m->get_order_by_payment(array('globalpaymentID' => $globalpaymentID, 'schoolID' => $schoolID));
                            $this->retdata['weaverandfines'] = pluck($this->weaverandfine_m->get_order_by_weaverandfine(array('globalpaymentID' => $globalpaymentID, 'schoolID' => $schoolID)), 'obj', 'paymentID');
    ...
    {
        $retArray = [];
        $invoiceitems = pluck_multi_array_key($this->invoice_m->get_order_by_invoice(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'maininvoiceID', 'invoiceID');
