...
class Hmember extends Api_Controller
{
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/hmember/view/{studentID}/{classID}",
       *     @OA\Parameter(
       *         name="studentID",
       *         description="",
       *         in = "path",
    ...
    public function view_get($id = null , $url = null)
    {
        ...
        if((int)$id && (int)$url) {
            ...
            if(isset($fetchClass[$url])) {
                ...
                $this->retdata['studentgroups'] = pluck($this->studentgroup_m->get_order_by_studentgroup(array('schoolID' => $schoolID)), 'group', 'studentgroupID');
                $this->retdata['optionalSubjects'] = pluck($this->subject_m->general_get_order_by_subject(array('type' => 0, 'schoolID' => $schoolID)), 'subject', 'subjectID');
