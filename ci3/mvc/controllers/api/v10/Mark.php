...
class Mark extends Api_Controller
{
                    ...
                    if(customCompute($this->retdata['students'])) {
                        ...
                        if(customCompute($sections)) {
                            foreach ($sections as $key => $section) {
                                $this->retdata['allsection'][$section->sectionID] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                            }
                        }
                    } else {
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/mark/view/{studentID}/{classID}",
       *     @OA\Parameter(
       *         name="studentID",
       *         description="",
       *         in = "path",
    ...
    {
        $this->retdata['subjects'] = pluck($this->subject_m->general_get_order_by_subject(array('schoolID' => $this->session->userdata('schoolID'))), 'subject', 'subjectID');
    }
