...
class Sattendance extends Api_Controller
{
	...
	public function index_get($id = null)
	{
		...
		if($this->session->userdata('usertypeID') == 3 && $myProfile) {
			...
			if((int)$id) {
				...
				$this->retdata['students'] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
				...
				if(isset($fetchClass[$id])) {
					if(customCompute($this->retdata['students'])) {
						...
						foreach ($sections as $key => $section) {
							$this->retdata['allsection'][$section->section] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
						}
					} else {
	...
	private function subjectattendance($id = null, $url = null)
	{
		...
		$attendances 		= $this->subjectattendance_m->get_order_by_sub_attendance(array("studentID" => $id, "classesID" => $url,'schoolyearID'=> $schoolyearID,'schoolID' => $schoolID));
		...
		$student           = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srclassesID' => $url, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
		$mandatorySubjects  = $this->subject_m->general_get_order_by_subject(array('type' => 1, 'classesID' => $url, 'schoolID' => $schoolID));
		if(customCompute($student)) {
            if($student->sroptionalsubjectID > 0) {
                $optionalSubject = $this->subject_m->general_get_order_by_subject(array('type' => 0, 'classesID' => $url, 'subjectID' => $student->sroptionalsubjectID, 'schoolID' => $schoolID));
	...
	/**
		 * @OA\Get(
		 *     path="/api/v10/sattendance/view/{studentID}/{classID}",
		 *     @OA\Parameter(
		 *         name="studentID",
		 *         description="",
		 *         in = "path",
	...
	public function view_get($id = null, $url = null)
	{
		...
		if((int)$id && (int)$url) {
			...
			$this->retdata["student"] = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srclassesID' => $url, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
			$this->retdata["classes"] = $this->classes_m->get_single_classes(array('classesID' => $url, 'schoolID' => $schoolID));
	...
	/**
* @OA\Post(
*   path="/api/v10/sattendance/add",
*   summary="Student attendance",
*   description="Add student attendance",
*   @OA\RequestBody(
*       required=true,
*       @OA\MediaType(
...
*               @OA\Property(
*                   property="subjectID",
*                   description="Academic Subject (optional)",
*                   type="string",
*                   example="1"
*               ),
*           )
...
*  @OA\Response(
*         response="400",
*         description="Invalid details submitted"
*     ),
*  @OA\Response(
*         response="404",
...
*/
	public function add_post()
	{
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if(customCompute($classesID) && $classesID != 0 && $this->data['siteinfos']->attendance == "subject") {
				$this->retdata['subjects'] = $this->subject_m->get_order_by_subject(array("classesID" => $classesID, 'schoolID' => $schoolID));
			} else {
				$this->retdata['subjects'] = [];
			}
      if(customCompute($classesID) && $classesID != 0) {
        $this->retdata['sections'] = $this->section_m->get_order_by_section(array("classesID" => $classesID, 'schoolID' => $schoolID));
			} else {
          ...
          $studentQuery = ['srschoolyearID' => $schoolyearID, "srclassesID" => $classesID, 'srsectionID' => $sectionID, 'srschoolID' => $schoolID];
          if($this->data['siteinfos']->attendance == "subject") {
          ...
          $students = $this->studentrelation_m->get_order_by_student($studentQuery);
					...
					if(customCompute($students)) {
						if($this->data['siteinfos']->attendance == "subject") {
							$attendance_monthyear = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID, "monthyear" => $monthyear, "schoolID" => $schoolID)), 'obj', 'studentID');
						} else {
							$attendance_monthyear = pluck($this->sattendance_m->get_order_by_attendance(array('schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "monthyear" => $monthyear, "schoolID" => $schoolID)), 'obj', 'studentID');
						}
						...
						if($this->data['siteinfos']->attendance == "subject") {
							$this->retdata['attendances'] = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('classesID' => $classesID, 'sectionID' => $sectionID, 'subjectID' => $subjectID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear, 'schoolID' => $schoolID)), 'obj', 'studentID');
						} else {
							$this->retdata['attendances'] = pluck($this->sattendance_m->get_order_by_attendance(array('classesID' => $classesID, 'sectionID' => $sectionID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear, 'schoolID' => $schoolID)), 'obj', 'studentID');
						}
					}
	...
	/**
* @OA\Post(
*   path="/api/v10/sattendance/saveattendance",
*   summary="Save student attendance",
...
*               @OA\Property(
*                   property="attendance[]",
*                   description="Present (P), Late w/ excuse (lE), late (L), and absent (A)",
*                   type="string",
*                   example="P"
*               ),
*           )
...
*  @OA\Response(
*         response="404",
*         description="Attendance data is not found"
*     ),
* )
*/
	public function saveattendance_post()
	{
        ...
        $attendance = json_decode($attendance, true);
				...
				if($f) {
					if($this->data['siteinfos']->attendance == "subject") {
						$data = array('a'.abs($day) => 'A', 'schoolyearID' => $schoolyearID, 'classesID' => $classesID, 'sectionID' => $sectionID,'monthyear' => $monthyear, 'subjectID' => $subjectID, 'schoolID' => $schoolID);
						$students = $this->subjectattendance_m->get_order_by_sub_attendance($data);
					} else {
						$data = array('a'.abs($day) => 'A', 'schoolyearID' => $schoolyearID, 'classesID' => $classesID, 'sectionID' => $sectionID, 'monthyear' => $monthyear, 'schoolID' => $schoolID);
	...
	private function sendAbsentEmail($students, $schoolyearID, $classesID, $sectionID)  // Complete
	{
		...
		$parents = pluck($this->parents_m->get_order_by_parents(array('schoolID' => $this->session->userdata('schoolID'))),'email','parentsID');
	...
	private function sendAbsentSMS($students, $schoolyearID, $classesID, $sectionID) // Complete
	{
		...
		$parents = pluck($this->parents_m->get_order_by_parents(array('schoolID' => $this->session->userdata('schoolID'))),'phone','parentsID');
