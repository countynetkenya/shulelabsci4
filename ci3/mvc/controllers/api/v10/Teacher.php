...
class Teacher extends Api_Controller
{
    ...
    public function index_get()
    {
        ...
        if($this->session->userdata('usertypeID') == 2 && $myProfile) {
            ...
            $teachers = $this->teacher_m->get_order_by_teacher(array('schoolID' => $this->session->userdata('schoolID')));
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/teacher/view/{teacherID}",
       *     @OA\Parameter(
       *         name="teacherID",
       *         description="",
       *         in = "path",
    ...
    {
        if((int)$teacherID) {
            $teacherInfo = $this->teacher_m->get_single_teacher(array('teacherID' => $teacherID, 'schoolID' => $this->session->userdata('schoolID')));
            $this->pluckInfo();
    ...
    private function pluckInfo()
    {
        ...
        $this->retdata['classess'] = pluck($this->classes_m->get_order_by_classes(array('schoolID' => $schoolID)), 'classes', 'classesID');
    ...
    private function routineInfo($teacherInfo)
    {
        ...
        if(customCompute($teacherInfo)) {
            ...
            $subject        = pluck($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'subjectID');
            $classes        = pluck($this->classes_m->general_get_order_by_classes(array('schoolID' => $schoolID)), 'obj', 'classesID');
            $section        = pluck($this->section_m->general_get_order_by_section(array('schoolID' => $schoolID)), 'obj', 'sectionID');
            $routines       = pluck_multi_array($this->routine_m->get_order_by_routine(array('teacherID'=>$teacherInfo->teacherID, 'schoolyearID'=> $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'day');
