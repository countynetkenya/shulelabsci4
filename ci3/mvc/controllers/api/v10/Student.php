...
class Student extends Api_Controller
{
            ...
            if((int)$id) {
                ...
                if(customCompute($this->retdata['students'])) {
                    ...
                    foreach ($sections as $key => $section) {
                        $this->retdata['allsection'][$section->sectionID] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
                    }
                } else {
    ...
    /**
       * @OA\Get(
       *     path="/api/v10/student/view/{studentID}/{classID}",
       *     @OA\Parameter(
       *         name="studentID",
       *         description="",
       *         in = "path",
    ...
    private function getView($id, $url)
    {
        ...
        if(isset($fetchClasses[$url])) {
            if((int)$id && (int)$url) {
                $studentInfo = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srclassesID' => $url, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID), TRUE);
                $this->pluckInfo();
    ...
    private function dayattendance($id = null, $url = null)
    {
        $schoolID           = $this->session->userdata('schoolID');
        $schoolyearID       = $this->session->userdata('defaultschoolyearID');
        $attendances        = $this->sattendance_m->get_order_by_attendance(array("studentID" => $id, "classesID" => $url,'schoolyearID'=> $schoolyearID,'schoolID' => $schoolID));
    ...
    private function pluckInfo()
    {
        ...
        $this->retdata['teachers'] = pluck($this->teacher_m->get_order_by_teacher(array('schoolID' => $schoolID)), 'name', 'teacherID');
    ...
    private function routineInfo($studentInfo)
    {
        ...
        if(customCompute($studentInfo)) {
            $schoolID = $studentInfo->srschoolID;
            $schoolyearID   = $this->session->userdata('defaultschoolyearID');
            $subject        = pluck(
                $this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)),
                'obj',
                'subjectID'
            );

            $teacher        = pluck(
                $this->teacher_m->get_select_teacher(NULL, array('schoolID' => $schoolID)),
                'obj',
                'teacherID'
            );
