...
*/
	...
	public function index() {
		...
		$this->data['templates'] = $this->certificate_template_m->get_order_by_certificate_template(array('schoolID' => $schoolID));
		...
		if(permissionChecker('certificatereport')) {
			if($_POST) {
			    ...
			    exit;
					...
					$sections = pluck($this->section_m->general_get_order_by_section(array('schoolID' => $schoolID)), 'section', 'sectionID');
					$classes = pluck($this->classes_m->general_get_order_by_classes(array('schoolID' => $schoolID)), 'classes', 'classesID');
      ...
      '2' => 'theme2'
		...
		if(permissionChecker('certificatereport')) {
			if((int)$userID && (int)$usertypeID && (int)$templateID && (int)$schoolyearID && (int)$classID) {
				$student = $this->studentrelation_m->general_get_single_student(array('srstudentID' => $userID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID), TRUE);

				$usertype = $this->usertype_m->get_single_usertype(array('usertypeID' => $usertypeID));
				$template = $this->certificate_template_m->get_single_certificate_template(array('certificate_templateID' => $templateID, 'schoolID' => $schoolID));
				...
				if(customCompute($student) && customCompute($usertype) && customCompute($template) && customCompute($schoolyear)) {
					...
					$tagClasses = $this->classes_m->general_get_single_classes(array('classesID' => $student->srclassesID, 'schoolID' => $schoolID));
					$tagSection = $this->section_m->general_get_single_section(array('sectionID' => $student->srsectionID, 'schoolID' => $schoolID));
					$tagGroup   = $this->studentgroup_m->get_single_studentgroup(array('studentgroupID' => $student->srstudentgroupID, 'schoolID' => $schoolID));
					$tagSubject = $this->subject_m->general_get_single_subject(array('subjectID' => $student->sroptionalsubjectID, 'schoolID' => $schoolID));
	...
	public function getSection() {
		...
		if((int)$id) {
			$allSection = $this->section_m->get_order_by_section(array('classesID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
