...
class Duefeesreport extends Admin_Controller{
	...
	public function getStudent() {
		...
		if((int)$classesID && (int)$sectionID && (int)$schoolyearID) {
			$students = $this->studentrelation_m->get_order_by_studentrelation(array('srclassesID' => $classesID,'srsectionID' => $sectionID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
		...
		if(permissionChecker('duefeesreport')) {
			if($_POST) {
			    ...
			    exit;
					...
					$studnetArray['srschoolID']    = $schoolID;
					...
					$this->data['getFeesReports'] = $this->totalPayment($this->payment_m->get_all_payment_for_report($this->input->post()), $schoolyearID, $schoolID);
	...
	private function totalPayment($arrays, $schoolyearID, $schoolID) {
		$weaverandfine = pluck($this->weaverandfine_m->get_order_by_weaverandfine(array('schoolyearID'=>$schoolyearID,'schoolID'=>$schoolID)),'obj','paymentID');
		$retArray = [];
