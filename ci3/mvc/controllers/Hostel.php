...
class Hostel extends Admin_Controller {
	...
	public function index() {
		$this->data['hostels'] = $this->hostel_m->get_order_by_hostel(array('schoolID' => $this->session->userdata('schoolID')));
	...
	public function edit() {
		...
		if((int)$id) {
			$this->data['hostel'] = $this->hostel_m->get_single_hostel(array('hostelID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
			if($this->data['hostel']) {
				if($_POST) {
					$rules = $this->rules();
					$this->form_validation->set_rules($rules);
	...
	public function delete() {
			$id = htmlentities(escapeString($this->uri->segment(3)));
			if((int)$id) {
				$hostel = $this->hostel_m->get_single_hostel(array("hostelID" => $id, 'schoolID' => $this->session->userdata('schoolID')));
				if(customCompute($hostel)) {
	...
	function unique_name() {
		...
		$schoolID = $this->session->userdata('schoolID');
		if((int)$id) {
			$student = $this->hostel_m->get_order_by_hostel(array("name" => $this->input->post("name"), "hostelID !=" => $id, 'schoolID' => $schoolID));
			...
			$student = $this->hostel_m->get_order_by_hostel(array("name" => $this->input->post("name"), 'schoolID' => $schoolID));
			if(customCompute($student)) {
				$this->form_validation->set_message("unique_name", "%s already exists");
				return FALSE;
			}
