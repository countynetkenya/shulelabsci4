...
*/
	...
	public function fileupload() {
		...
		if((int)$id) {
			...
			$syllabus = $this->syllabus_m->get_single_syllabus(array('syllabusID' => $id, 'schoolID' => $schoolID));
		}
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1))  {
				$this->data['classes'] = $this->classes_m->get_order_by_classes(array("schoolID" => $schoolID));
				$this->data['syllabus'] = $this->syllabus_m->get_single_syllabus(array('syllabusID' => $id, 'classesID' => $url, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
				$fetchClasses = pluck($this->data['classes'], 'classesID', 'classesID');
	...
	public function delete() {
		...
		if((int)$id && (int)$url) {
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1))  {
				...
				if(isset($fetchClasses[$url])) {
					$syllabus = $this->syllabus_m->get_single_syllabus(array('syllabusID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
					if(customCompute($syllabus)) {
