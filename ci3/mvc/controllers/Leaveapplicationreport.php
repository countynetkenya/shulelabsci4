...
*/
    ...
    public function index() {
        ...
        $this->data['leavecategories'] = $this->leavecategory_m->get_order_by_leavecategory(array('schoolID' => $schoolID));
            ...
            if($_POST) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if($this->input->post('categoryID')) {
                        $leavecategory = $this->leavecategory_m->get_single_leavecategory(array('leavecategoryID'=>$this->input->post('categoryID'),'schoolID'=>$schoolID));
                    ...
                    $this->data['userObejct']   = getAllUserObjectWithStudentRelation(array('schoolyearID' => $this->session->userdata('defaultschoolyearID'), 'schoolID' => $schoolID), TRUE);
    ...
    public function pdf() {
        if(permissionChecker('leaveapplicationreport')) {
            ...
            if(((int)$usertypeID || $usertypeID == 0) && ((int)$classesID || $classesID == 0) && ((int)$sectionID || $sectionID == 0) && ((int)$userID || $userID == 0) && ((int)$categoryID || $categoryID == 0) && ((int)$statusID || $statusID == 0) && ((int)$fromdate || $fromdate == null) && ((int)$todate || $todate == null)) {
                ...
                if($categoryID) {
                    $leavecategory = $this->leavecategory_m->get_single_leavecategory(array('leavecategoryID' => $categoryID, 'schoolID' => $schoolID));
                    if(customCompute($leavecategory)) {
                        $this->data['categoryName'] = $leavecategory->leavecategory;
                    } else {
                        $this->data['categoryName'] = '';
    ...
    private function queryArray($posts) {
        ...
        if($fromdate && $todate) {
            ...
            $queryArray['todate'] = $todate." 23:59:59";
				...
				$queryArray['schoolID']     = $this->session->userdata('schoolID');
    ...
    public function getUser() {
        ...
        if((int)$usertypeID) {
            ...
            if($usertypeID == 1) {
                ...
                $users = $this->teacher_m->general_get_order_by_teacher(array('schoolID' => $schoolID));
                if(customCompute($users)) {
                    foreach ($users as $user) {
