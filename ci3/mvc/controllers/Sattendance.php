...
*/
	public function __construct() {
		...
		$this->data['setting'] = $this->setting_m->get_setting($this->session->userdata('schoolID'));
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			$schoolID = $this->session->userdata('schoolID');
			$this->schoolyear_m->get_obj_schoolyear(array('schoolID' => $schoolID));
			$this->data['headerassets'] = array(
				'css' => array(
					'assets/select2/css/select2.css',
					'assets/select2/css/select2-bootstrap.css',
			...
			if($classesID != 0 && $this->data['setting']->attendance == "subject") {
				$this->data['subjects'] = $this->subject_m->get_order_by_subject(array("classesID" => $classesID, 'schoolID' => $schoolID));
			} else {
				$this->data['subjects'] = [];
			}
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if($this->data['setting']->attendance == "subject") {
						...
						$subjectInfo =  $this->subject_m->get_single_subject(array('subjectID' => $subjectID, 'schoolID' => $schoolID));
					...
					$students = $this->studentrelation_m->get_order_by_student(array('srschoolyearID' => $schoolyearID, "srclassesID" => $classesID, 'srsectionID' => $sectionID, 'srschoolID' => $schoolID));

					$studentArray = [];
					$this->data['attendances'] = [];
					if(customCompute($students)) {
						if($this->data['setting']->attendance == "subject") {
							$attendance_monthyear = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID, "monthyear" => $monthyear)), 'obj', 'studentID');
						} else {
							$attendance_monthyear = pluck($this->sattendance_m->get_order_by_attendance(array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "monthyear" => $monthyear)), 'obj', 'studentID');
						}
						...
						if($this->data['setting']->attendance == "subject") {
							$this->data['attendances'] = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('schoolID' => $schoolID, 'classesID' => $classesID, 'sectionID' => $sectionID, 'subjectID' => $subjectID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear)), 'obj', 'studentID');
						} else {
							$this->data['attendances'] = pluck($this->sattendance_m->get_order_by_attendance(array('schoolID' => $schoolID, 'classesID' => $classesID, 'sectionID' => $sectionID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear)), 'obj', 'studentID');
						}
					}
					...
					$this->data['sattendanceinfo']['class'] = $this->classes_m->get_single_classes(array('classesID' => $classesID, 'schoolID' => $schoolID))->classes;
					$this->data['sattendanceinfo']['section'] = $this->section_m->get_single_section(array('sectionID' => $sectionID, 'schoolID' => $schoolID))->section;
	...
	public function save_attendace() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($this->data['setting']->attendance == "subject") {
					$data = array('a'.abs($day) => "A", 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID,'classesID'=>$classes,'sectionID' => $sectionID,'monthyear'=>$monthyear,'subjectID'=>$subjectID);
					$students = $this->subjectattendance_m->get_order_by_sub_attendance($data);
				} else {
					$data = array('a'.abs($day) => "A", 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID,'classesID'=>$classes,'sectionID' => $sectionID,'monthyear'=>$monthyear);
					$students = $this->sattendance_m->get_order_by_attendance($data);
				}

				if($f && customCompute($students)) {
					if($messageType == 'email') {
						$this->sendAbsentEmail($students, $schoolID, $schoolyearID, $classes, $sectionID);
					} elseif($messageType == 'sms') {
						$this->sendAbsentSMS($students, $schoolID, $schoolyearID, $classes, $sectionID);
					}
				}
	...
	private function sendAbsentEmail($students, $schoolID, $schoolyearID, $classesID, $sectionID) {
		...
		$objStudents = pluck($this->studentrelation_m->get_order_by_student(array('schoolID' => $schoolID, 'srschoolyearID'=> $schoolyearID, 'srclassesID'=> $classesID,'srsectionID'=> $sectionID), TRUE),'obj','srstudentID');

		$parents = pluck($this->parents_m->get_order_by_parents(array('schoolID' => $schoolID)),'email','parentsID');
	...
	private function sendAbsentSMS($students, $schoolID, $schoolyearID, $classesID, $sectionID) {
		$attendance_smsgateway = $this->data['setting']->attendance_smsgateway;
		$templateID = $this->data['setting']->attendance_notification_template;
		$mailandsmstemplate = $this->mailandsmstemplate_m->get_mailandsmstemplate($templateID);
		$objStudents = pluck($this->studentrelation_m->get_order_by_student(array('schoolID' => $schoolID, 'srschoolyearID' => $schoolyearID, 'srclassesID' => $classesID, 'srsectionID' => $sectionID), TRUE),'obj','srstudentID');

		$parents = pluck($this->parents_m->get_order_by_parents(array('schoolID' => $schoolID)),'phone','parentsID');
				...
				if($userTag->tagname == '[name]') {
					...
					if($sendType == 'email') {
						if($user->usertypeID == 3) {
							$result = $this->resultTableEmail($user->srstudentID, $user->srclassesID, $schoolyearID, $user->srschoolID);
						} else {
							$result = '';
						}
						...
						if($user->usertypeID == 3) {
							$result = $this->resultTableSMS($user->srstudentID, $user->srclassesID, $schoolyearID, $user->srschoolID);
						} else {
							$result = '';
						}
						$message = str_replace("[result_table]", $result, $message);
	...
	private function resultTableEmail($studentID, $classesID, $schoolyearID, $schoolID) {
		$string = '<br>';
		$markArray = $this->getMarkArray($studentID, $classesID, $schoolyearID, $schoolID);
	...
	private function resultTableSMS($studentID, $classesID, $schoolyearID, $schoolID) {
		$string = ' ';
		$markArray = $this->getMarkArray($studentID, $classesID, $schoolyearID, $schoolID);
	...
	private function getMarkArray($studentID, $classesID, $schoolyearID, $schoolID) {
		$this->load->model('markpercentage_m');
		$this->load->model('markrelation_m');

		if((int)$studentID && (int)$classesID) {
			$this->data['generate']["student"] 				= $this->studentrelation_m->get_single_student(array('srstudentID' => $studentID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID), TRUE);
			$this->data['generate']["classes"] 				= $this->classes_m->get_single_classes(array('classesID' => $classesID, 'schoolID' => $schoolID));
			if($this->data['generate']["student"] && $this->data['generate']["classes"]) {
				$this->data['generate']['set'] 				= $classesID;
				$this->data['generate']["exams"] 			= $this->exam_m->get_order_by_exam(array('schoolID' => $schoolID));
				$this->data['generate']["grades"] 			= $this->grade_m->get_order_by_grade(array('schoolID' => $schoolID));
				$this->data['generate']['markpercentages']	= $this->markpercentage_m->get_order_by_markpercentage(array('schoolID' => $schoolID));
				$this->data['generate']['mandatorysubjects'] = $this->subject_m->general_get_order_by_subject(array('classesID' => $classesID, 'type' => 1, 'schoolID' => $schoolID));
				$this->data['generate']['allsubjects'] = pluck($this->subject_m->general_get_order_by_subject(array('classesID' => $classesID, 'schoolID' => $schoolID)), 'subject', 'subjectID');

				$allMarkWithRelation = $this->markrelation_m->get_all_mark_with_relation(array('classesID' => $classesID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
	...
	private function get_setting_mark_percentage() {
		$schoolID = $this->session->userdata('schoolID');
		$markpercentagesDatabases = $this->markpercentage_m->get_order_by_markpercentage(array('schoolID' => $schoolID));
	...
	public function view($id = null, $url = null) {
		...
		if((int)$id && (int)$url) {
			...
			if($this->data['setting']->attendance == "subject") {
				$this->data["subjects"] = $this->subject_m->general_get_order_by_subject(array("classesID" => $url, 'schoolID' => $schoolID));
			}

			$this->data["student"] = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srclassesID' => $url, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
			$this->data["classes"] = $this->classes_m->get_single_classes(array('classesID' => $url, 'schoolID' => $schoolID));
	...
	public function subjectall() {
		...
		$id = $this->input->post('id');
		if((int)$id) {
			$allsubject = $this->subject_m->get_order_by_subject(array("classesID" => $id, 'schoolID' => $schoolID));
	...
	public function sectionall() {
		...
		if((int)$classesID) {
			$sections = $this->section_m->get_order_by_section(array('classesID' => $classesID, 'schoolID' => $schoolID));
