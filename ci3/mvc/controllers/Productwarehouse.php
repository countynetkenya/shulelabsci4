...
class Productwarehouse extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        $this->data['productwarehouses'] = $this->productwarehouse_m->get_order_by_productwarehouse(array('schoolID' => $this->session->userdata('schoolID')));
    ...
    public function unique_productwarehousename() {
        ...
        if((int)$id) {
            $productwarehouse = $this->productwarehouse_m->get_order_by_productwarehouse(array("productwarehousename" => $this->input->post("productwarehousename"), 'productwarehousecode' => $this->input->post('productwarehousecode'), "productwarehouseID !=" => $id, 'schoolID' => $schoolID));
            ...
            $productwarehouse = $this->productwarehouse_m->get_order_by_productwarehouse(array("productwarehousename" => $this->input->post("productwarehousename"), 'productwarehousecode' => $this->input->post('productwarehousecode'), 'schoolID' => $schoolID));
            if(customCompute($productwarehouse)) {
                $this->form_validation->set_message("unique_productwarehousename", "The %s is already exists.");
                return FALSE;
            }
    ...
    public function edit() {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $this->data['productwarehouse'] = $this->productwarehouse_m->get_single_productwarehouse(array('productwarehouseID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
    ...
    public function unique_warehousecode() {
        ...
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $productwarehouse = $this->productwarehouse_m->get_order_by_productwarehouse(array("productwarehousecode" => $this->input->post("productwarehousecode"), "productwarehouseID !=" => $id, "schoolID" => $schoolID));
            if(customCompute($productwarehouse)) {
            ...
            $productwarehouse = $this->productwarehouse_m->get_order_by_productwarehouse(array("productwarehousecode" => $this->input->post("productwarehousecode"), "schoolID" => $schoolID));
