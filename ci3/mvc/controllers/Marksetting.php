...
*/
	...
	public function index() {
		...
		$this->data['subjects']        = pluck_multi_array($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'classesID');
		$this->data['markpercentages'] = pluck($this->markpercentage_m->get_order_by_markpercentage(array('schoolID' => $schoolID)), 'obj', 'markpercentageID');

		$marksetting                   = $this->marksetting_m->get_marksetting_with_marksettingrelation(array('schoolID' => $schoolID));
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$marktypeID = $this->input->post('marktypeID');
				$this->setting_m->insertorupdate($schoolID, ['marktypeID'=> $marktypeID]);
				...
				if($marktypeID == 0) {
					...
					if(customCompute($exams)) {
						...
						foreach ($exams as $exam) {
							...
							$marksettingArr[$i]['schoolID']   = $schoolID;
							$i++;
						}
					}

					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 0, 'schoolID'=> $schoolID]);
					...
					while ($j < $marksettingCount) {
						if(customCompute($markpercentages)) {
							foreach ($markpercentages as $markpercentage) {
								...
								$marksettingRelationArr[$i]['schoolID']         = $schoolID;
								$i++;
							}
						}
						$j++; $marksettingID++;
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 0, 'schoolID'=> $schoolID]);
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 1, 'schoolID'=> $schoolID]);
					$exams = $this->input->post('exams');
					if(customCompute($exams)) {
						$j=0;
						foreach ($exams as $exam) {
							...
							if(customCompute($markpercentageArr)) {
								foreach ($markpercentageArr as $classesID => $markpercentages) {
									...
									$marksettingArr['schoolID']   = $schoolID;
									...
									if(customCompute($markpercentages)) {
										foreach ($markpercentages as $markpercentage) {
											...
											$marksettingRelationArr[$j]['schoolID']         = $schoolID;
											$j++;
										}
									}
								}
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 1, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				} elseif($marktypeID == 2) {
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 2, 'schoolID'=> $schoolID]);
					$j = 0;
					if(customCompute($markpercentageArr)) {
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 2, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				} elseif($marktypeID == 3) {
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 3, 'schoolID'=> $schoolID]);
					$j = 0;
					if(customCompute($markpercentageArr)) {
						foreach ($markpercentageArr as $examID => $markpercentages) {
							$marksettingArr['examID']     = $examID;
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 3, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				} elseif($marktypeID == 4) {
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 4, 'schoolID'=> $schoolID]);
					$i = 0;
					if(customCompute($markpercentageArr)) {
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 4, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				} elseif($marktypeID == 5) {
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 5, 'schoolID'=> $schoolID]);
					$j=0;
					if(customCompute($markpercentageArr)) {
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 5, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				} elseif($marktypeID == 6) {
					...
					$this->marksetting_m->delete_marksetting_by_array(['marktypeID'=> 6, 'schoolID'=> $schoolID]);
					$j=0;
					if(customCompute($markpercentageArr)) {
					...
					$this->marksettingrelation_m->delete_marksettingrelation_by_array(['marktypeID'=> 6, 'schoolID'=> $schoolID]);
					if(customCompute($marksettingRelationArr)) {
						$this->marksettingrelation_m->insert_batch_marksettingrelation($marksettingRelationArr);
					}
				}
	...
	public function check_markpercentage() {
		...
		$markpercentages   = $this->input->post('markpercentages');
		$schoolID          = $this->session->userdata('schoolID');
		$markpercentageArr = pluck($this->markpercentage_m->get_order_by_markpercentage(array('schoolID' => $schoolID)), 'obj', 'markpercentageID');
		if($marktypeID == 0) {
			// Global
			$totalmark     = 0;
			if(customCompute($markpercentages)) {
			...
			// Class Wise
			$ex_class   = $this->data['siteinfos']->ex_class;
			$classes    = $this->classes_m->general_get_order_by_classes(['classesID !='=> $ex_class, 'schoolID'=> $schoolID]);
			...
			// Subject Wise
			$subjects   = pluck_multi_array_key($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'classesID', 'subjectID');
			$ex_class   = $this->data['siteinfos']->ex_class;
			$classes    = pluck($this->classes_m->general_get_order_by_classes(['classesID !='=> $ex_class, 'schoolID'=> $schoolID]), 'obj', 'classesID');
			...
			$ex_class   = $this->data['siteinfos']->ex_class;
			$classes    = pluck($this->classes_m->general_get_order_by_classes(['classesID !='=> $ex_class, 'schoolID'=> $schoolID]), 'obj', 'classesID');
			$exams      = pluck($this->exam_m->get_order_by_exam(array('schoolID' => $schoolID)), 'obj', 'examID');
			...
			$classes    = pluck($this->classes_m->general_get_order_by_classes(['classesID !='=> $ex_class, 'schoolID'=> $schoolID]), 'obj', 'classesID');
			$exams      = pluck($this->exam_m->get_order_by_exam(array('schoolID' => $schoolID)), 'exam', 'examID');
			$subjects   = pluck($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'subject', 'subjectID');
			$subjectClass   = pluck_multi_array($this->subject_m->general_get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'classesID');
