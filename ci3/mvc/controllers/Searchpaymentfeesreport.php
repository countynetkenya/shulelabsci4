...
class Searchpaymentfeesreport extends Admin_Controller {
	...
	public function getSearchPaymentFeesReport() {
		...
		if(permissionChecker('searchpaymentfeesreport')) {
            ...
            if((int)$gspaymentID && (($gspayment == 'INV-G-') || ($gspayment == 'inv-g-'))) {
                $this->data['globalpayments'] = $this->payment_m->get_order_by_payment(array('globalpaymentID' => $gspaymentID, 'schoolID' => $schoolID));
                ...
                if(customCompute($globalpayments)) {
                    ...
                    $this->data['studentinfo'] = $this->studentrelation_m->get_single_studentrelation(array('srschoolyearID' => $globalschoolyearID, 'srstudentID' => $studentID, 'srschoolID' => $schoolID));
                    ...
                    $this->data['weaverandfines'] = pluck($this->weaverandfine_m->get_order_by_weaverandfine(array('globalpaymentID'=>$gspaymentID,'schoolID'=>$schoolID)),'obj','paymentID');
                }
            	...
            	$this->data['singlepayments'] = $this->payment_m->get_single_payment_by_globalpaymentID(array('globalpaymentID' => $gspaymentID, 'schoolID' => $schoolID));
                if(customCompute($this->data['singlepayments'])) {
                    ...
                    $this->data['feetypes'] = pluck($this->feetypes_m->get_order_by_feetypes(array('schoolID' => $schoolID)),'feetypes','feetypesID');
                    $this->data['studentinfo'] = $this->studentrelation_m->get_single_studentrelation(array('srschoolyearID' => $globalschoolyearID, 'srstudentID' => $studentID, 'srschoolID' => $schoolID));
                    $this->data["student"] = $this->student_m->get_single_student(array('studentID' => $studentID, 'schoolID' => $schoolID));
                }
	...
	public function pdf() {
		if(permissionChecker('searchpaymentfeesreport')) {
            ...
            if((int)$gspaymentID && (($gspayment == 'INV-G-') || ($gspayment == 'inv-g-'))) {
                $this->data['globalpayments'] = $this->payment_m->get_globalpayments(array('globalpaymentID' => $gspaymentID, 'schoolID' => $schoolID));
                $globalpayments = $this->data['globalpayments'];
                if(customCompute($globalpayments)) {
                    $studentID = $globalpayments[0]->studentID;
                    $globalschoolyearID = $globalpayments[0]->schoolyearID;
