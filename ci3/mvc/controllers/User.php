...
*/
	...
	protected function rules() {
		$rules = array(
			...
			array(
				'field' => 'schoolID[]',
				'label' => $this->lang->line("user_schoolID"),
				'rules' => 'trim|required|xss_clean|callback_valid_schoolID'
			)
		);
		return $rules;
	}
	...
	public function photoupload() {
		...
		if((int)$id) {
			...
			$user = $this->user_m->get_single_user(array('userID' => $id, 'schoolID' => $schoolID));
		}
	...
	public function index() {
		$schoolID = $this->session->userdata('schoolID');
		$usertype = pluck($this->usertype_m->get_order_by_usertype_with_or(array('schoolID' => $schoolID)), 'obj', 'usertypeID');
		...
		if(isset($usertype[$this->session->userdata('usertypeID')]) && $myProfile) {
			...
			$this->data['users'] = $this->user_m->get_user_by_usertype(array('schoolID' => $schoolID));
	...
	public function add() {
		...
		$this->data['schools']   = $this->school_m->get_school_wherein(explode(",", $this->session->userdata('schools')));
		if($_POST) {
			$rules = $this->rules();
			...
			if ($this->form_validation->run() == FALSE) {
				...
				/*if($this->input->post("usertypeID") >= 5 && $this->input->post("usertypeID") <= 8)
					$array["schoolID"] = $user->schoolID;
				else*/
				$array['schoolID'] = implode(",", $this->input->post("schoolID"));
	...
	public function edit() {
		...
		$this->data['schools'] = $this->school_m->get_school_wherein(explode(",", $this->session->userdata('schools')));
		if((int)$id) {
			$this->data['user'] = $this->user_m->get_user_by_usertype(array('userID' => $id, 'schoolID' => $schoolID));
			if($this->data['user']) {
				$rules = $this->rules();
				unset($rules[11]);
				$this->form_validation->set_rules($rules);
	...
	private function getView($userID) {
		if ((int)$userID) {
			$schoolID = $this->session->userdata('schoolID');
			$users = $this->user_m->get_user_by_usertype(array('userID' => $userID, 'schoolID' => $schoolID));
			$this->pluckInfo();
	...
	public function attendanceInfo($users) {
		if(customCompute($users)) {
			...
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$schoolID = $this->session->userdata('schoolID');
			$uattendances = $this->uattendance_m->get_order_by_uattendance(array("userID" => $users->userID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
	...
	private function salaryInfo($users) {
		if(customCompute($users)) {
						$schoolID     = $this->session->userdata('schoolID');
            $manageSalary = $this->manage_salary_m->get_single_manage_salary(array('usertypeID' => $users->usertypeID, 'userID' => $users->userID, 'schoolID' => $schoolID));
	...
	public function print_preview() {
		...
		if(permissionChecker('user_view') || ((isset($usertype[$this->session->userdata('usertypeID')])) && permissionChecker('user') && ($this->session->userdata('loginuserID') == htmlentities(escapeString($this->uri->segment(3)))))) {
			...
			if((int)$userID) {
				$this->data['user'] = $this->user_m->get_user_by_usertype(array('userID' => $userID, 'schoolID' => $schoolID));
	...
	public function valid_schoolID()
	{
			...
			if(customCompute($schoolID)) {
					foreach($schoolID as $value) {
							$result = $this->systemadmin_m->get_systemadmin_by_usertype(array('systemadminID' => $this->session->userdata('loginuserID'), 'schoolID' => $value));
	...
	public function active() {
		if(permissionChecker('user_edit')) {
			...
			if($id != '' && $status != '') {
				if((int)$id) {
					$schoolID = $this->session->userdata('schoolID');
					$user = $this->user_m->get_user_by_usertype(array('userID' => $id, 'schoolID' => $schoolID));
					if(customCompute($user)) {
						if($status == 'chacked') {
