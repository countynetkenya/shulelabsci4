...
*/
	...
	public function index() {
		...
		$this->data['fmenus'] = $this->fmenu_m->get_order_by_fmenu(array('schoolID' => $schoolID));
		$this->data['activefmenu'] = $this->fmenu_m->get_single_fmenu(array('status' => 1, 'schoolID' => $schoolID));
		$this->data['pages'] = $this->pages_m->get_order_by_pages(array('schoolID' => $schoolID));
		$this->data['posts'] = $this->posts_m->get_order_by_posts(array('schoolID' => $schoolID));
		...
		if($_POST) {
			...
			if(customCompute($pages)) {
				foreach ($pages as $page) {
					...
					$pageID = $expPage[1];
					$pageArrays[] = $this->pages_m->get_single_pages(array('pagesID' => $pageID, 'schoolID' => $this->session->userdata('schoolID')));
				}
		...
		if($_POST) {
			...
			if(customCompute($posts)) {
				foreach ($posts as $post) {
					...
					$postID = $expPost[1];
					$postArrays[] = $this->posts_m->get_single_posts(array('postsID' => $postID, 'schoolID' => $this->session->userdata('schoolID')));
				}
	...
	public function managelocation() {
		$schoolID = $this->session->userdata('schoolID');
		$fmenus = $this->fmenu_m->get_order_by_fmenu(array('schoolID' => $schoolID));
		if($_POST) {
			if(customCompute($this->fmenu_m->get_order_by_fmenu(array('schoolID' => $schoolID)))) {
				...
				if(customCompute($elements)) {
					...
					if(customCompute($validation)) {
						...
						if(customCompute($elements)) {
							...
							if($locationtop) {
								$this->fmenu_m->update_fmenu_by_array(array('topbar' => 0), array('topbar' => 1, 'schoolID' => $schoolID));
							}

							if($locationsocial) {
								$this->fmenu_m->update_fmenu_by_array(array('social' => 0), array('social' => 1, 'schoolID' => $schoolID));
							}
		...
		if($_POST) {
			...
			if($fmenuID != '') {
				if((int)$fmenuID) {
					...
					$fmenu = $this->fmenu_m->get_single_fmenu(array('fmenuID' => $fmenuID, 'schoolID' => $schoolID));
	...
	public function deletefmenu() {
		if($_POST) {
			...
			if($fmenuID != '') {
				if((int)$fmenuID) {
					$fmenu = $this->fmenu_m->get_single_fmenu(array('fmenuID' => $fmenuID, 'schoolID' => $this->session->userdata('schoolID')));
					if(customCompute($fmenu)) {
		...
		if($_POST) {
			...
			if(!empty($menuName)) {
				...
				if(strlen($menuName) <= 128) {
					$schoolID = $this->session->userdata('schoolID');
					$fmenu = $this->fmenu_m->get_order_by_fmenu(array('schoolID' => $schoolID));
					if(customCompute($fmenu)) {
						$this->fmenu_m->update_fmenu_by_array(array('status' => 0), array('status' => 1, 'schoolID' => $schoolID));
					}

					$this->fmenu_m->insert_fmenu(array('menu_name' => $menuName, 'status' => 1, 'topbar' => 0, 'social' => 0, 'schoolID' => $schoolID));
					$retArray['status'] = TRUE;
					$retArray['message'] = $this->lang->line('menu_success');
				} else {
