...
*/
	...
	public function index() {
		...
		if($this->session->userdata('usertypeID') == 2 && $myProfile) {
			...
			$this->data['teachers'] = $this->teacher_m->general_get_teacher(array('schoolID' => $schoolID));
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if(customCompute($teachers)) {
						...
						$this->data['tattendances'] = pluck($this->tattendance_m->get_order_by_tattendance(array("monthyear" => $monthyear, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'teacherID');
	...
	public function view($id = null) {
		...
		if((int)$id) {
			...
			$this->data["teacher"] = $this->teacher_m->general_get_single_teacher(array('teacherID' => $id, 'schoolID' => $schoolID));
			if(customCompute($this->data["teacher"])) {
				...
				$tattendances = $this->tattendance_m->get_order_by_tattendance(array("teacherID" => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
