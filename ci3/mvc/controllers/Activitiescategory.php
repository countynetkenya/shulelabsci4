...
*/
	...
	public function index() {
		$this->data['activitiescategorys'] = $this->activitiescategory_m->get_order_by_activitiescategory(array('schoolID' => $this->session->userdata('schoolID')));
	...
	public function edit() {
		...
		if((int)$id) {
			$this->data['activitiescategory'] = $this->activitiescategory_m->get_single_activitiescategory(array('activitiescategoryID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function delete() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$activitiescategory = $this->activitiescategory_m->get_single_activitiescategory(array('activitiescategoryID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function unique_activitiescategory() {
		...
		$schoolID = $this->session->userdata('schoolID');
		if((int)$id) {
			$activitiescategory = $this->activitiescategory_m->get_order_by_activitiescategory(array("title" => $this->input->post("title"), "activitiescategoryID !=" => $id, "schoolID" => $schoolID));
			if(customCompute($activitiescategory)) {
			...
			$activitiescategory = $this->activitiescategory_m->get_order_by_activitiescategory(array("title" => $this->input->post("title"), "schoolID" => $schoolID));
			if(customCompute($activitiescategory)) {
				$this->form_validation->set_message("unique_activitiescategory", "%s already exists");
				return FALSE;
			}
