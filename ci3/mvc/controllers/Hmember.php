...
*/
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if($hostelID > 0) {
				$this->data['categorys'] = $this->category_m->get_order_by_category(array("hostelID" => $hostelID, 'schoolID' => $schoolID));
			} else {
				$this->data['categorys'] = [];
			}

			if((int)$id && (int)$url) {
				$student = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
					...
					if($student->hostel == 0) {
						if($_POST) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$hostel_main_id = $this->hostel_m->get_single_hostel(array('hostelID' => $this->input->post("hostelID"), 'schoolID' => $schoolID));
								$category_main_id = $this->category_m->get_single_category(array("hostelID" => $hostel_main_id->hostelID, "categoryID" =>  $this->input->post("categoryID"), 'schoolID' => $schoolID));
								if($hostel_main_id) {
									if($category_main_id) {
										$array = array(
											"hostelID" => $this->input->post("hostelID"),
		...
		if(permissionChecker('hmember_view') || (($this->session->userdata('usertypeID') == 3) && permissionChecker('hmember') && ($this->session->userdata('loginuserID') == $this->input->post('studentID')))) {
	        ...
	        $this->data['optionalSubjects'] = pluck($this->subject_m->general_get_order_by_subject(array('type' => 0, 'schoolID' => $schoolID)), 'subject', 'subjectID');
					...
					if ((int)$id && (int)$url) {
						$fetchClass = pluck($this->classes_m->get_order_by_classes(array('schoolID' => $schoolID)), 'classesID', 'classesID');
						if(isset($fetchClass[$url])) {
							$this->data["student"] = $this->studentrelation_m->get_single_student(array('srschoolyearID' => $schoolyearID, 'srstudentID' => $id, 'srschoolID' => $schoolID));
							$this->data['usertypes'] = pluck($this->usertype_m->get_order_by_usertype_with_or(array('schoolID' => $schoolID)),'usertype','usertypeID');
							if(customCompute($this->data["student"])) {
	...
	public function categorycall() {
		...
		if((int)$classtype) {
			$allclasstype = $this->category_m->get_order_by_category(array("hostelID" => $classtype, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function unique_gender() {
		...
		if((int)$id) {
			if($this->input->post("hostelID") == 0) {
				...
				$schoolID = $this->session->userdata('schoolID');
				$student = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
				$hostel = $this->hostel_m->get_single_hostel(array("hostelID" => $this->input->post("hostelID"), 'schoolID' => $schoolID));
				if($hostel) {
					$gender = "";
					if($student->sex == "Male") {
						$gender = "Boys";
	...
	public function unique_category() {
		...
		if($hostelID != 0 && $categoryID != 0 ) {
			$category = $this->category_m->get_single_category(array('hostelID' => $hostelID, 'categoryID' => $categoryID, 'schoolID' => $this->session->userdata('schoolID')));
