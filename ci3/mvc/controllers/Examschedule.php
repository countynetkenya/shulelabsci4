...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			$this->data['examschedules'] = $this->examschedule_m->get_join_examschedule_with_exam_classes_section_subject(array('classesID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
			if($this->data['examschedules']) {
				...
				foreach ($sections as $key => $section) {
					$this->data['allsection'][$section->section] = $this->examschedule_m->get_join_examschedule_with_exam_classes_section_subject(array('classesID' => $id, 'sectionID' => $section->sectionID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
				}
			} else {
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id && (int)$url) {
				...
				if(isset($fetchClass[$url])) {
					$this->data['examschedule'] = $this->examschedule_m->get_single_examschedule(array('examscheduleID' => $id, 'classesID' => $url, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
					if(customCompute($this->data['examschedule'])) {
						...
						$this->data['subjects'] = $this->subject_m->general_get_order_by_subject(array('classesID' => $classID, 'schoolID' => $schoolID));
						$this->data['sections'] = $this->section_m->general_get_order_by_section(array("classesID" => $classID, 'schoolID' => $schoolID));
						$this->data['exams']    = $this->marksetting_m->get_exam(array('marktypeID' => $this->data['siteinfos']->marktypeID, 'classesID' => $classID, 'schoolID' => $schoolID));
						$this->data['set']      = $url;
	...
	public function check_duplicate_subject() {
		if($_POST) {
			...
			$qArray['schoolID']     = (int)$this->session->userdata('schoolID');
