...
*/
    ...
    public function photoupload()
    {
        ...
        $user = [];
        if ( (int) $id ) {
            $user = $this->teacher_m->get_single_teacher([ 'teacherID' => $id, 'schoolID' => $schoolID ]);
        }
    ...
    public function edit()
    {
        ...
        if ( (int) $id ) {
            $schoolID              = $this->session->userdata('schoolID');
            $this->data['teacher'] = $this->teacher_m->get_single_teacher([ 'teacherID' => $id , 'schoolID' => $schoolID ]);
    ...
    private function getView( $teacherID )
    {
        if ( (int) $teacherID ) {
            $schoolID    = $this->session->userdata('schoolID');
            $teacherinfo = $this->teacher_m->get_single_teacher([ 'teacherID' => $teacherID , 'schoolID' => $schoolID ]);
            $this->pluckInfo();
    ...
    {
        $schoolID               = $this->session->userdata('schoolID');
        $this->data['subjects'] = pluck($this->subject_m->general_get_subject(array('schoolID' => $schoolID)), 'subject', 'subjectID');
        $this->data['classess'] = pluck($this->classes_m->get_classes(array('schoolID' => $schoolID)), 'classes', 'classesID');
        $this->data['sections'] = pluck($this->section_m->get_section(array('schoolID' => $schoolID)), 'section', 'sectionID');
    }
    ...
    public function download_document()
    {
        ...
        if ( (int) $documentID && (int) $teacherID ) {
            if ( ( permissionChecker('teacher_add') && permissionChecker('teacher_delete') ) || ( $this->session->userdata('usertypeID') == 2 && $this->session->userdata('loginuserID') == $teacherID ) ) {
                $schoolID = $this->session->userdata('schoolID');
                $document = $this->document_m->get_single_document([ 'documentID' => $documentID , 'schoolID' => $schoolID ]);
    ...
    private function examInfo($teacherInfo) {
  		if(customCompute($teacherInfo)) {
        $schoolID = $this->session->userdata('schoolID');
        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        $teacherID = $teacherInfo->teacherID;
        ...
        $classes                         = $this->classes_m->general_get_single_classes(array('teacherID' => $teacherID, 'schoolID' => $schoolID));
        ...
        $teacherSubjects                 = $this->subject_m->get_order_by_subject_with_subjectteacher(array('classesID' => $classesID, 'teacherID'=>$teacherID, 'type' => 1, 'schoolID' => $schoolID));
        $this->data['teacher']           = $this->teacher_m->general_get_single_teacher(array('teacherID' => $teacherID, 'schoolID' => $schoolID));
        $teachers                        = $this->teacher_m->get_order_by_teacher_with_subjectteacher(array('classesID' => $classesID));
        $this->data['grades']            = $this->grade_m->get_grade(array('schoolID' => $schoolID));
        $this->data['percentageArr']     = pluck($this->markpercentage_m->get_markpercentage(array('schoolID' => $schoolID)), 'obj', 'markpercentageID');
        $exams                           = $this->exam_m->get_last_six_exams_with_subject($teacherID);
        $examID 												 = [];
        if(customCompute($exams)) {
          foreach($exams as $exam)
  					$examID[] = $exam->examID;
          $marks                         = $this->mark_m->get_order_by_all_student_mark_with_markrelation2(['schoolyearID' => $schoolyearID, 'classesID' => $classesID, 'examID' => $examID, 'schoolID' => $schoolID]);
        }
        $marks             							 = $this->getMark($marks);
        $this->data['marks']             = $marks;

        $markpercentagesmainArr          = $this->marksetting_m->get_marksetting_markpercentages(array('schoolID' => $schoolID));
        ...
        if ( permissionChecker('teacher_view') || ( ( $this->session->userdata('usertypeID') == 2 ) && permissionChecker('teacher') && ( $this->session->userdata('loginuserID') == $this->input->post('teacherID') ) ) ) {
            if ( $_POST ) {
                ...
                if ( $this->form_validation->run() == false ) {
                    ...
                    if ( (int) $teacherID ) {
                        $schoolID              = $this->session->userdata('schoolID');
                        $this->data["teacher"] = $this->teacher_m->get_single_teacher([ 'teacherID' => $teacherID , 'schoolID' => $schoolID ]);
                        if ( $this->data["teacher"] ) {
    ...
    public function active()
    {
        if ( permissionChecker('teacher_edit') ) {
            ...
            if ( $id != '' && $status != '' ) {
                if ( (int) $id ) {
                    $schoolID = $this->session->userdata('schoolID');
                    $teacher = $this->teacher_m->get_single_teacher([ 'teacherID' => $id , 'schoolID' => $schoolID ]);
