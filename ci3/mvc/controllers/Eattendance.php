...
*/
	...
	public function index() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->data['eattendances'] = pluck($this->eattendance_m->get_order_by_eattendance(array("examID" => $examID, 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, "subjectID" => $subjectID)), 'obj', 'studentID');
				...
				if(customCompute($this->data['students'])) {
					...
					foreach ($sections as $key => $section) {
						$this->data['allsection'][$section->section] = $this->studentrelation_m->get_order_by_student(array('srschoolID' => $schoolID, 'srschoolyearID' => $schoolyearID, 'srclassesID' => $classesID, "srsectionID" => $section->sectionID));
					}
				}
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$students = $this->studentrelation_m->get_order_by_student(array('srschoolID' => $schoolID, 'srschoolyearID' => $schoolyearID, "srclassesID" => $classesID, 'srsectionID' => $sectionID));
					$eattendance = pluck($this->eattendance_m->get_order_by_eattendance(array("examID" => $examID, 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID)), 'obj', 'studentID');
					...
					$this->data['eattendances'] = pluck($this->eattendance_m->get_order_by_eattendance(array("examID" => $examID, 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID)), 'obj', 'studentID');
					...
					$this->data['eattendanceinfo']['exam'] = $this->exam_m->get_single_exam(array('examID' => $examID, 'schoolID' => $schoolID));
					$this->data['eattendanceinfo']['class'] = $this->classes_m->general_get_single_classes(array('classesID' => $classesID, 'schoolID' => $schoolID));
					$this->data['eattendanceinfo']['section'] = $this->section_m->general_get_single_section(array('sectionID' => $sectionID, 'schoolID' => $schoolID));
					$this->data['eattendanceinfo']['subject'] = $this->subject_m->general_get_single_subject(array('subjectID' => $subjectID, 'schoolID' => $schoolID));
					$this->data["subview"] = "eattendance/add";
					$this->load->view('_layout_main', $this->data);
				}
			} else {
	...
	public function sectioncall() {
		...
		if((int)$classID) {
			$schoolID = $this->session->userdata('schoolID');
			$sections = $this->section_m->general_get_order_by_section(array("classesID" => $classID, "schoolID" => $schoolID));
	...
	public function single_add() {
		...
		if((int)$examID && (int)$classesID && (int)$subjectID) {
			...
			$this->eattendance_m->update_eattendance_via_array($array, array("examID" => $examID, 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID, "studentID" => $studentID));
	...
	public function all_add() {
		...
		if((int)$examID && (int)$classesID && (int)$subjectID) {
			$array = array("eattendance" => $status);
			$this->eattendance_m->update_eattendance_via_array($array, array("examID" => $examID, 'schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, "classesID" => $classesID, 'sectionID' => $sectionID, "subjectID" => $subjectID));
			echo $this->lang->line('menu_success');
		}
	}
}
