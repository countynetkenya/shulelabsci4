...
*/
	...
	public function photoupload() {
		...
		$user = array();
		if((int)$id) {
			$user = $this->systemadmin_m->get_single_systemadmin(array('systemadminID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
		}
	...
	public function index() {
		$this->data['systemadmins'] = $this->systemadmin_m->get_systemadmin_by_usertype(array('schoolID' => $this->session->userdata('schoolID')));
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$array['schoolID'] = $systemadmin->schoolID;
	...
	public function edit() {
		...
		if ((int)$id) {
			...
			$this->data['systemadmin'] = $this->systemadmin_m->get_systemadmin_by_usertype(array('systemadminID' => $id, 'schoolID' => $schoolID));
	...
	private function getView($systemadminID) {
		if((int)$systemadminID) {
			$schoolID = $this->session->userdata('schoolID');
			$systemadmin = $this->systemadmin_m->get_systemadmin_by_usertype(array('systemadminID' => $systemadminID, 'schoolID' => $schoolID));
			$this->pluckInfo();
	...
	private function pluckInfo() {
		$this->data['usertypes'] = pluck($this->usertype_m->get_order_by_usertype_with_or(array('schoolID' => $this->session->userdata('schoolID'))),'usertype','usertypeID');
	}
	...
	public function delete_document() {
		...
		if((int)$documentID && (int)$systemadminID) {
			if(permissionChecker('systemadmin_add') && permissionChecker('systemadmin_delete')) {
				...
				if(customCompute($document)) {
					if(config_item('demo') == FALSE) {
						if(file_exists(FCPATH.'uploads/documents/'.$document->file)) {
							unlink(FCPATH.'uploads/documents/'.$document->file);
						}
					}
		...
		if(permissionChecker('systemadmin_view')) {
			if($_POST) {
			    ...
			    exit;
					...
					if ((int)$systemadminID) {
						$schoolID = $this->session->userdata('schoolID');
						$this->data['systemadmin'] = $this->systemadmin_m->get_systemadmin_by_usertype(array('systemadminID' => $systemadminID, 'schoolID' => $schoolID));
						if(customCompute($this->data["systemadmin"])) {
							$this->data['panel_title'] = $this->lang->line('panel_title');
