 ...
 */
    ...
    public function index()
    {
        ...
        $this->data['overtimes'] = $this->overtime_m->get_order_by_overtime(array('schoolID' => $schoolID));
        $this->data['roles']     = pluck($this->usertype_m->get_order_by_usertype_with_or(array('schoolID' => $schoolID)), 'usertype', 'usertypeID');
        $this->data['allUsers']  = getAllUserObjectWithoutStudent(array('schoolID' => $schoolID));
    ...
    public function add()
    {
        ...
        $schoolID            = $this->session->userdata('schoolID');
        $this->data['roles'] = $this->usertype_m->get_usertype_for_overtime(array('schoolID' => $schoolID));
        ...
        if ($userID > 0) {
            ...
            $this->data['users'] = $this->user_m->get_all_user($table, array('usertypeID' => $roleId, 'schoolID' => $schoolID));
        } else {
    ...
    public function edit()
    {
        ...
        if ((int) $id) {
            $this->data['overtime'] = $this->overtime_m->get_single_overtime(array('id' => $id, 'schoolID' => $schoolID));
    ...
    public function delete()
    {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $id) {
            $overtime = $this->overtime_m->get_single_overtime(array('id' => $id, 'schoolID' => $this->session->userdata('schoolID')));
            if(customCompute($overtime)) {
    ...
    public function userscall()
    {
        ...
        if ($roleId) {
            ...
            $schoolID     = $this->session->userdata('schoolID');
            $getUsers     = $this->user_m->get_all_user($table, array('usertypeID' => $roleId, 'schoolID' => $schoolID));
            $manageSalary = pluck_multi_array_key($this->manage_salary_m->get_order_by_manage_salary(['salary' => 1, 'schoolID' => $schoolID]), 'obj', 'usertypeID', 'userID');
    ...
    private function hourCalculation($roleId, $userId, $hours)
    {
        if ((int) $roleId && (int) $userId && $hours > 0) {
            $schoolID = $this->session->userdata('schoolID');
            $manageSalary = $this->manage_salary_m->get_single_manage_salary(['usertypeID' => $roleId, 'userID' => $userId, 'schoolID' => $schoolID]);
            if (is_object($manageSalary)) {
                $salaryTemplete = $this->salary_template_m->get_single_salary_template(['salary_templateID' => $manageSalary->template, 'schoolID' => $schoolID]);
