...
class Vendor extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        $this->data['vendors'] = $this->vendor_m->get_order_by_vendor(array('schoolID' => $this->session->userdata('schoolID')));
    ...
    public function unique_email() {
        if($this->input->post('email')) {
            ...
            if((int)$id) {
                $vendor = $this->vendor_m->get_single_vendor(array('email' => $this->input->post('email'), 'vendorID !=' => $id, 'schoolID' => $schoolID));
                ...
                $vendor = $this->vendor_m->get_single_vendor(array('email' => $this->input->post('email'), 'schoolID' => $schoolID));
                if(customCompute($vendor)) {
                    $this->form_validation->set_message("unique_email", "The %s is already exists.");
                    return FALSE;
                }
    ...
    public function edit() {
        ...
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $this->data['vendor'] = $this->vendor_m->get_single_vendor(array('vendorID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
            if($this->data['vendor']) {
                if($_POST) {
                    $rules = $this->rules();
                    $this->form_validation->set_rules($rules);
