	...
	public function rules() {
		$rules = array(
			...
			array(
				...
				'label'=>$this->lang->line('balancefeesreport_group'),
				'rules' => 'trim|xss_clean'
			),
			array(
				'field'=>'studentID',
			...
			if($_POST) {
			  ...
			  if ($this->form_validation->run() == FALSE) {
			    ...
			    exit;
					...
					$this->data['schooltermID']   = $schooltermID;
					...
					foreach ($this->data['students'] as $student) {
						if($this->studentgroup_m->get_single_studentgroup(array('studentgroupID' => $student->srstudentgroupID)) != null)
					...
					$this->data['totalAmountAndDiscountBf'] = $this->totalAmountAndDiscustomCompute($this->invoice_m->get_all_balancefees_bf_for_report($this->input->post()));
					$this->data['totalCredit'] = $this->totalAmountAndDiscustomCompute($this->creditmemo_m->get_all_balancefees_for_report($this->input->post()));
					$this->data['totalCreditBf'] = $this->totalAmountAndDiscustomCompute($this->creditmemo_m->get_all_balancefees_bf_for_report($this->input->post()));

					$paymentArray['schoolyearID'] = $schoolyearID;
					$paymentBfArray['schoolID']   = $schoolID;

					if (isset($schoolterm) && strtotime($schoolterm->startingdate)) {
						$paymentArray['paymentdate >=']  = $schoolterm->startingdate;
						$paymentBfArray['paymentdate <'] = $schoolterm->startingdate;
					}
					elseif (isset($schoolterm) && strtotime($schoolterm->endingdate))
						$paymentArray['paymentdate <='] = $schoolterm->endingdate;
					elseif (strtotime($dateFrom)) {
						$paymentArray['paymentdate >='] = $dateFrom;
						$paymentBfArray['paymentdate <'] = $dateFrom;
					}
					elseif (strtotime($dateTo))
						$paymentArray['paymentdate <='] = $dateTo;
					elseif (strtotime($dateTo))
						$paymentArray['paymentdate <='] = $dateTo;

					if (!isset($schoolterm) && !strtotime($dateFrom))
						$paymentBfArray = array('schoolyearID <'=>$schoolyearID);

					$this->data['totalPayment'] = $this->totalPaymentAndWeaver($this->payment_m->get_order_by_payment($paymentArray));
					$this->data['totalPaymentBf'] = $this->totalPaymentAndWeaver($this->payment_m->get_order_by_payment($paymentBfArray));
	...
	public function pdf() {
		if(permissionChecker('balancefeesreport')) {
			...
			if((int)($classesID >= 0) || (int)($sectionID >= 0) || (int)($studentID >= 0)) {
				...
				$this->data['totalPayment'] = $this->totalPaymentAndWeaver($this->payment_m->get_order_by_payment(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID)));
				$this->data['totalweavar'] = $this->totalWeaver($this->weaverandfine_m->get_order_by_weaverandfine(array('schoolyearID'=>$schoolyearID, 'schoolID' => $schoolID)));
