...
class Studentmeritlistreport extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->lang->load('studentmeritlistreport', $language);
	}
	...
	public function index() {
		...
		$this->data["subview"] = "report/studentmeritlist/StudentmeritlistReportView";
  ...
  protected function rules() {
		$rules = array(
			array(
				'field'=>'examID[]',
				'label'=>$this->lang->line('studentmeritlistreport_exam'),
				'rules' => 'trim|required|xss_clean|callback_multiple_select'
			),
			array(
				'field'=>'classesID',
				'label'=>$this->lang->line('studentmeritlistreport_class'),
				'rules' => 'trim|xss_clean|numeric'
			),
			array(
				'field'=>'sectionID',
				'label'=>$this->lang->line('studentmeritlistreport_section'),
				'rules' => 'trim|xss_clean|numeric'
			),
			array(
				'field'=>'subjectID',
				'label'=>$this->lang->line('studentmeritlistreport_subject'),
				'rules' => 'trim|xss_clean|numeric'
			)
		);
		return $rules;
  ...
  public function getStudentmeritlistReport() {
		$retArray['render'] = '';
		$retArray['status'] = FALSE;
		if(permissionChecker('studentmeritlistreport')) {
			...
			if($_POST) {
			    ...
			    exit;
					...
					$queryArray   = array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID);
					$studentQuery = array('srschoolID' => $schoolID, 'srschoolyearID' => $schoolyearID);
					$markQuery    = array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID);
          ...
          $this->data['marks']             = $marks;
						...
						foreach ($exams as $exam) {
							if(customCompute($students)) {
								foreach ($students as $student) {
									$markpercentagesArr     				 = isset($markpercentagesmainArr[$student->srclassesID][$exam->examID]) ? $markpercentagesmainArr[$student->srclassesID][$exam->examID] : [];
											...
											if(customCompute($markpercentages)) {
												foreach ($markpercentages as $markpercentageID) {
													...
													if(isset($studentPosition[$exam->examID][$student->srstudentID]['subjectMark'][$mandatorySubject->subjectID])) {
														if(isset($marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID])) {
													...
													if(isset($marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID])) {
														$studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$mandatorySubject->subjectID] = $marks[$exam->examID][$student->srstudentID][$mandatorySubject->subjectID][$markpercentageID];

														if(isset($studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'])) {
															$studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'] += $studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$mandatorySubject->subjectID];
														} else {
															$studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'] = $studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$mandatorySubject->subjectID];
														}
													}

													if(!isset($studentChecker['subject'][$exam->examID][$student->srstudentID])) {
														if($student->sroptionalsubjectID != 0) {
															...
															if(isset($marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID])) {
																$studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$student->sroptionalsubjectID] = $marks[$exam->examID][$student->srstudentID][$student->sroptionalsubjectID][$markpercentageID];

																if(isset($studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'])) {
																	$studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'] += $studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$student->sroptionalsubjectID];
																} else {
																	$studentPosition[$exam->examID][$student->srstudentID]['markpercentagetotalmark'] = $studentPosition[$exam->examID][$student->srstudentID]['markpercentageMark'][$student->sroptionalsubjectID];
																}
															}
														}
														$studentChecker['subject'][$exam->examID][$student->srstudentID] = TRUE;
													}
												}
											}
					...
					$retArray['render'] = $this->load->view('report/studentmeritlist/StudentmeritlistReport',$this->data,true);
  ...
  public function getSubject() {
      ...
      if(customCompute($subjects)) {
        foreach ($subjects as $value) {
  				echo "<option value=\"$value->subjectID\">",$value->subject," (", $value->name ,")</option>";
  			}
      }
		}
	}
