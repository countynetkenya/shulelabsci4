...
class Complain extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        if($usertypeID == 1) {
            $this->data['complains'] = $this->complain_m->get_order_by_complain(array('schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
            $this->data["subview"] = "complain/index";
            $this->load->view('_layout_main', $this->data);
        } else {
            $this->data['complains'] = $this->complain_m->get_order_by_complain(array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, 'create_userID' => $userID, 'create_usertypeID' => $usertypeID));
            $this->data["subview"] = "complain/index";
            $this->load->view('_layout_main', $this->data);
        }
    }
        ...
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if((int)$id) {
                ...
                if($loginusertypeID == 1) {
                    $this->data['complain'] = $this->complain_m->get_single_complain(array('complainID' => $id, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                } else {
                    $this->data['complain'] = $this->complain_m->get_single_complain(array('complainID' => $id, 'create_usertypeID' => $loginusertypeID, 'create_userID' => $loginuserID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID));
                }
                if(customCompute($this->data['complain'])) {
                    ...
                    if($usertypeID != 0) {
                        if($usertypeID == 1) {
                            $this->data['users'] = pluck($this->systemadmin_m->get_order_by_systemadmin(array('schoolID' => $schoolID)), 'name', 'systemadminID');
                        } elseif($usertypeID == 2) {
                            $this->data['users'] = pluck($this->teacher_m->get_order_by_teacher(array('schoolID' => $schoolID)), 'name', 'teacherID');
                        } elseif($usertypeID == 3) {
                            $student = $this->studentrelation_m->general_get_single_student(array('srstudentID' => $this->data['userID'], 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
                            ...
                            $students = $this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $this->data['classesID'], 'srschoolID' => $schoolID));
                            $studentArray = [];
                            ...
                            $this->data['users'] = pluck($this->parents_m->get_order_by_parents(array('schoolID' => $schoolID)), 'name', 'parentsID');
                        } else {
                            $this->data['users'] = pluck($this->user_m->get_order_by_user(array('usertypeID' => $usertypeID, 'schoolID' => $schoolID)), 'name', 'userID');
                        }
                    } else {
    ...
    public function view() {
        ...
        if((int)$id) {
            if($loginusertypeID == 1) {
                ...
                $this->data['complain'] = $this->complain_m->get_single_complain(array('complainID' => $id, 'schoolyearID' => $schoolyearID, 'create_userID' => $loginuserID, 'create_usertypeID' => $loginusertypeID, 'schoolID' => $schoolID));
            }
            if(customCompute($this->data['complain'])) {
                $usertypeID = $this->data['complain']->usertypeID;
                $userID     = $this->data['complain']->userID;
                ...
                if($usertypeID > 0 && $userID > 0) {
                    ...
                    if((int)$usertypeID) {
                        if($usertypeID == 1) {
                            $this->data['user'] = $this->systemadmin_m->get_single_systemadmin(array('systemadminID'=> $userID, 'schoolID' => $schoolID));
                        } elseif($usertypeID == 2) {
                            $this->data['user'] = $this->teacher_m->get_single_teacher(array('teacherID'=> $userID, 'schoolID' => $schoolID));
                        } elseif($usertypeID == 3) {
                            $this->data['user'] = $this->studentrelation_m->general_get_single_student(array('srstudentID'=> $userID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
                            $this->data['classes'] = $this->classes_m->general_get_single_classes(array('classesID'=>$this->data['user']->srclassesID, 'schoolID' => $schoolID));
                            $this->data['section'] = $this->section_m->general_get_single_section(array('sectionID'=>$this->data['user']->srsectionID, 'schoolID' => $schoolID));
                        } elseif($usertypeID == 4) {
                            $this->data['user'] = $this->parents_m->get_single_parents(array('parentsID'=> $userID, 'schoolID' => $schoolID));
                        } else {
                            $this->data['user'] = $this->user_m->get_single_user(array('usertypeID' => $usertypeID, 'userID'=> $userID, 'schoolID' => $schoolID));
                        }
                    } else {
                        $this->data['user'] = [];
                        $this->data['classes'] = [];
    ...
    public function download() {
        ...
        if((int)$id) {
            $schoolID = $this->session->userdata('schoolID');
            $complain = $this->complain_m->get_single_complain(array('complainID' => $id, 'schoolID' => $schoolID));
            if(customCompute($complain)) {
