    ...
    */
    ...
    public function index() {
        ...
        if($usertypeID == '3') {
            ...
            $this->data['opsubject'] = pluck($this->subject_m->get_order_by_subject(['type'=> 0, 'schoolID' => $schoolID]), 'obj', 'subjectID');
        }

        $this->data['usertypeID']   = $usertypeID;
        $this->data['online_exams'] = $this->online_exam_m->get_order_by_online_exam(array('schoolYearID' => $schoolyearID, 'schoolID' => $schoolID));
    ...
    public function add() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            $this->data['instructions'] = $this->instruction_m->get_order_by_instruction(array('schoolID' => $schoolID));
            $this->data['types'] = $this->exam_type_m->get_order_by_exam_type(['status' => 1, 'schoolID' => $schoolID]);
            ...
            if($_POST) {
                $this->data['sections'] = $this->section_m->general_get_order_by_section(array('classesID' => $this->input->post('classes'), 'schoolID' => $schoolID));
                $this->data['subjects'] = $this->subject_m->general_get_order_by_subject(array('classesID' => $this->input->post('classes'), 'schoolID' => $schoolID));
    ...
    public function edit() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            $schoolYearID = $this->session->userdata('defaultschoolyearID');
            if((int)$id) {
                $this->data['online_exam'] = $this->online_exam_m->get_single_online_exam(array('onlineExamID' => $id, 'schoolYearID' => $schoolYearID, 'schoolID' => $schoolID));
    ...
    public function delete() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if((int)$id) {
                $schoolYearID = $this->session->userdata('defaultschoolyearID');
                $this->data['online_exam'] = $this->online_exam_m->get_single_online_exam(array('onlineExamID' => $id, 'schoolYearID' => $schoolYearID, 'schoolID' => $this->session->userdata('schoolID')));
    ...
    public function addquestion() {
        ...
        if(permissionChecker('online_exam_add')) {
            ...
            if((int)$onlineExamID) {
                $schoolID = $this->session->userdata('schoolID');
                $onlineExam = $this->online_exam_m->get_single_online_exam(['onlineExamID' => $onlineExamID, 'schoolID' => $schoolID]);
                if(customCompute($onlineExam)) {
                    $this->data['onlineExamID'] = $onlineExamID;
                    $this->addQuestionDatabase($schoolID, true, $onlineExamID);
                    $this->data['levels'] = $this->question_level_m->get_order_by_question_level(array('schoolID' => $schoolID));
                    $this->data['groups'] = $this->question_group_m->get_order_by_question_group(array('schoolID' => $schoolID));
    ...
    public function showQuestions() {
        ...
        $this->data['types'] = pluck($this->question_type_m->get_order_by_question_type(array('schoolID' => $schoolID)), 'obj', 'typeNumber');

        echo $this->load->view('/online_exam/questionList', $this->data, true);
    }

    public function addQuestionDatabase($schoolID, $initial = false, $onlineExamID = 0, $questionID = 0)
    {
        if(!$initial) {
        ...
        $this->data['questions'] = pluck($this->question_bank_m->get_order_by_question_bank(array('schoolID' => $schoolID)), 'obj', 'questionBankID');
        $this->data['levels'] = pluck($this->question_level_m->get_order_by_question_level(array('schoolID' => $schoolID)),'obj', 'questionLevelID');


        $allOptions = $this->question_option_m->get_order_by_question_option(array('schoolID' => $schoolID));
        $options = [];
        ...
        $allAnswers = $this->question_answer_m->get_order_by_question_answer(array('schoolID' => $schoolID));
        $answers = [];
    ...
    public function removeQuestionDatabase()
    {
        ...
        $onlineExamID = $this->input->post('onlineExamID');
        $online_exam_question = $this->online_exam_question_m->get_single_online_exam_question(array('onlineExamQuestionID' => $onlineExamQuestionID, 'schoolID' => $schoolID));
        if(customCompute($online_exam_question)) {
    ...
    public function getSubject()
    {
        ...
        if((int)$classID) {
            $allSubject = $this->subject_m->general_get_order_by_subject(array('classesID' => $classID, 'schoolID' => $this->session->userdata('schoolID')));
    ...
    public function check_exam_question() {
        ...
        if((int)$onlineexamID) {
            $online_exam_questions = $this->online_exam_question_m->get_order_by_online_exam_question(array('onlineExamID'=>$onlineexamID, 'schoolID' => $this->session->userdata('schoolID')));
