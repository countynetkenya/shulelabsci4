...
class Bulkimport extends Admin_Controller
{
    ...
    */
                        ...
                        foreach($csv_array as $row) {
                            ...
                            if(customCompute($match) <= 0) {
                                ...
                                if($singlestudentCheck['status']) {
                                    ...
                                    $classes = $this->classes_m->general_get_single_classes(['classesID' => $classID, 'schoolID' => $schoolID]);
    ...
    public function student_with_parent_bulkimport()
    {
        if(isset($_FILES["csvStudentWithParent"])) {
            $config['upload_path']   = "./uploads/csv/";
            $config['allowed_types'] = 'text/plain|text/csv|csv';
            $config['max_size']      = '2048';
            $config['file_name']     = $_FILES["csvStudentWithParent"]['name'];
            $config['overwrite']     = TRUE;
            $this->load->library('upload', $config);
            if(!$this->upload->do_upload("csvStudentWithParent")) {
                $this->session->set_flashdata('error', $this->lang->line('bulkimport_upload_fail'));
                            ...
                            if(customCompute($match) <= 0) {
                                ...
                                if($singleparentCheck['status']) {
                                    $insert_data = [
                                        ...
                                        'username'          => $row['Guardian Username'],
                                        'password'          => $this->parents_m->hash($row['Guardian Password']),
                                        'usertypeID'        => 2,
                                        'photo'             => 'default.png',
                                        "create_date"       => date("Y-m-d h:i:s"),
                                        "modify_date"       => date("Y-m-d h:i:s"),
                                ...
                                if($singlestudentCheck['status']) {
                                    ...
                                    $group           = $this->get_student_group($row['Group']);
                                    $optionalSubject = $this->get_student_optional_subject($classID, $row['OptionalSubject']);
                                    $dob             = $this->trim_required_convertdate($row['Birthday Date']);
                                    $admissiondate   = $this->trim_required_convertdate($row['Admission Date']);
    ...
    private function singleteacherCheck( $array )
    {
        ...
        if($name && $designation && $dob && $gender && $religion && $email && $phone && $address && $jod && $username && $password) {
            ...
            if(!$religion) {
                $retArray['error']['religion'] = 'Invalid Religion';
            }
            if(!$email) {
    ...
    {
        $name              = $this->trim_required_string_maxlength_minlength_Check($array['guardian_name'], 60);
        ...
        $email             = $this->trim_check_unique_email($array['guardian_email'], 40);
        $phone             = $this->trim_required_string_maxlength_minlength_Check($array['guardian_phone'], 25, 5);
        $address           = $this->trim_required_string_maxlength_minlength_Check($array['guardian_address'], 200);
        $username          = $this->trim_check_unique_username($array['guardian_username'], 40, 4);
        $password          = $this->trim_required_string_maxlength_minlength_Check($array['guardian_password'], 40, 4);
    ...
    {
        $name            = $this->trim_required_string_maxlength_minlength_Check($array['name'], 60);
        $admissiondate   = $this->trim_required_date_Check($array['admission_date']);
        $dob             = $this->trim_required_date_Check($array['birthday_date']);
        $gender          = $this->trim_required_string_maxlength_minlength_Check($array['gender'], 10);
        $religion        = $this->trim_required_string_maxlength_minlength_Check($array['religion'], 25);
        $email           = $this->trim_check_unique_email($array['email'], 40);
        $phone           = $this->trim_required_string_maxlength_minlength_Check($array['phone'], 25, 5);
        ...
        $retArray['status'] = FALSE;
        if($name && $admissiondate && $dob && $gender && $religion && $email && $phone && $address && $class && $section && $username && $password && $bloodgroup && $state && $country && $group && $checkStudent) {
            $retArray['status'] = TRUE;
        } else {
            if(!$name) {
                $retArray['error']['name'] = 'Invalid Teacher Name';
            ...
            if(!$admissiondate) {
                $retArray['error']['admission_date'] = 'Invalid Date Of Admission';
            }
            if(!$dob) {
                $retArray['error']['dob'] = 'Invalid Date Of Birth';
            }
    ...
    private function trim_check_section_student( $array )
    {
        ...
        if($classes && $section) {
            $result = $this->classes_m->general_get_single_classes(['LOWER(classes)' => $classes, 'schoolID' => $schoolID]);
    ...
    private function trim_required_class_Check( $classes )
    {
        $classes = strtolower(trim($classes));
        if($classes) {
            $result = $this->classes_m->general_get_single_classes(['LOWER(classes)' => $classes, 'schoolID' => $this->session->userdata('schoolID')]);
            if(customCompute($result)) {
                return $result;
            }
        }
    ...
    private function trim_group_Check( $group )
    {
        ...
        if($group1 && $group2) {
            $schoolID = $this->session->userdata('schoolID');
            $result1  = $this->studentgroup_m->get_single_studentgroup(['group' => $group1, 'schoolID' => $schoolID]);
            $result2  = $this->studentgroup_m->get_single_studentgroup(['group' => $group2, 'schoolID' => $schoolID]);
    ...
    private function get_id( $data, $type )
    {
        $schoolID = $this->session->userdata('schoolID');
        if($type == 'group') {
            $group = $this->question_group_m->get_single_question_group(['title' => $data, 'schoolID' => $schoolID]);
            if($group) {
                return $group->questionGroupID;
            }
            return false;
        } elseif($type == 'level') {
            $level = $this->question_level_m->get_single_question_level(['name' => $data, 'schoolID' => $schoolID]);
            if($level) {
            ...
            $type = $this->question_type_m->get_single_question_type(['name' => $data, 'schoolID' => $schoolID]);
            if($type) {
