...
*/
	...
	protected function markRules() {
		$rules = array(
			...
			array(
				'field' => 'comments',
				'label' => $this->lang->line("mark_subject"),
				'rules' => 'trim|xss_clean|max_length[11]|callback_unique_comments'
			)
		);
		return $rules;
	}
		...
		if($usertypeID == 3 && $myProfile) {
			...
			if((int)$id) {
				...
				if(isset($fetchClass[$id])) {
					$studentArray = array('srclassesID' => $id, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID);
					if($usertypeID == 3)
						$studentArray['srstudentID'] = $this->session->userdata('loginuserID');
					$this->data['students'] = $this->studentrelation_m->get_order_by_student($studentArray);
					if(customCompute($this->data['students'])) {
						...
						foreach ($sections as $key => $section) {
							$this->data['allsection'][$section->sectionID] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
						}
					} else {
						$this->data['students'] = [];
					}
	...
	public function add() {
      ...
      $classesID = $this->input->post("classesID");
			$schoolID  = $this->session->userdata('schoolID');
      if((int)$classesID) {
      	  $this->data['exams']    = $this->marksetting_m->get_exam(array('marktypeID' => $this->data['siteinfos']->marktypeID, 'classesID' => $classesID, 'schoolID' => $schoolID));
      ...
      $this->data['classes']  = $this->classes_m->get_order_by_classes(['classesID !='=> $graduateclass, 'schoolID' => $schoolID]);
      ...
      if($_POST) {
          ...
          if ($this->form_validation->run() == FALSE) {
              ...
              $exam            = $this->exam_m->get_single_exam(array('examID'=> $examID, 'schoolID'=> $schoolID));
              $subject         = $this->subject_m->get_single_subject(array('subjectID'=> $subjectID, 'schoolID'=> $schoolID));
              $classes         = $this->classes_m->get_single_classes(array('classesID'=> $classesID, 'schoolID'=> $schoolID));
							$section         = $this->section_m->get_single_section(array('sectionID'=> $sectionID, 'schoolID'=> $schoolID));
		      		...
		      		$markpercentageArr['subject']    = $subject;
							$markpercentageArr['schoolID']    = $schoolID;
              ...
              if(customCompute($subject)) {
              	if($subject->type == 1) {
	                ...
	                $students = $this->studentrelation_m->get_order_by_student([
	                    "srclassesID"    	=> $classesID,
	                    'srschoolyearID' 	=> $schoolyearID,
											'srnonexaminablesubjectID' => $subject->subjectID,
											'srschoolID'      => $schoolID,
	                ]);

									$studentArray['srnonexaminablesubjectID'] = $subject->subjectID;
              	} else {
              ...
              $markPluck   = pluck($this->mark_m->get_order_by_mark(array("examID" => $examID, "classesID" => $classesID, "	subjectID" => $subjectID, 'schoolyearID' => $schoolyearID, 'schoolID' => $schoolID)), 'obj', 'studentID');
              ...
              $array = [];
                  ...
                  $mark = $this->mark_m->get_order_by_mark(array('schoolyearID' => $schoolyearID, "examID" => $examID, "classesID" => $classesID, "subjectID" => $subjectID, "schoolID" => $schoolID));
                  $this->data['marks'] = $mark;
              }

							if(customCompute($students)) {
								$missingmMarkRelationArray = [];
								$allMarkWithRelation = $this->markrelation_m->get_all_mark_with_relation(array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, 'examID' => $examID, 'classesID' => $classesID, 'subjectID' => $subjectID));
							...
							$this->data['markRelations']    = $this->getMarkRelationArray($this->mark_m->student_all_mark_array(array('schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, 'examID' => $examID, 'classesID' => $classesID, 'subjectID' => $subjectID)));
	...
	public function view($studentID = null, $classID = null) {
		...
		if((int) $studentID && (int) $classID) {
			$schoolID     = $this->session->userdata('schoolID');
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$student = $this->studentrelation_m->get_single_student(array('srstudentID' => $studentID, 'srclassesID' => $classID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
	...
	private function getView($id, $url)
  {
		if((int)$id && (int)$url) {
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$schoolID = $this->session->userdata('schoolID');
			$studentInfo = $this->studentrelation_m->get_single_student(array('srstudentID' => $id, 'srclassesID' => $url, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
	...
	private function getMark($studentID, $classesID) {
		if((int)$studentID && (int)$classesID) {
			$schoolID 		= $this->session->userdata('schoolID');
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$student      = $this->studentrelation_m->get_single_student(array('srstudentID' => $studentID, 'srclassesID' => $classesID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
			$classes      = $this->classes_m->get_single_classes(array('classesID' => $classesID, 'schoolID' => $schoolID));
			...
			if(customCompute($student) && customCompute($classes)) {
				...
				$marksettings  = $this->marksetting_m->get_marksetting_markpercentages(array('schoolID' => $schoolID));
          ...
          exit;
					...
					if(customCompute($comments)) {
						foreach ($comments as $key => $value) {
							$data = explode('-', $value['comment']);
							if(!empty($value['value']) || $value['value'] != "") {
								$markArray[] = [
									'markID' => $data[1],
									'teacher_comment' => $value['value']
								];
							} else {
								$markArray[] = [
									'markID' => $data[1],
					...
					if(customCompute($markArray)) {
						$this->mark_m->update_batch_mark($markArray, 'markID');
					}
	...
	private function getMarkPrintPDF($studentID, $classesID) {
		if((int)$studentID && (int)$classesID) {
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$schoolID 		= $this->session->userdata('schoolID');
			$student      = $this->studentrelation_m->get_single_student(array('srstudentID' => $studentID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
			$classes      = $this->classes_m->get_single_classes(array('classesID' => $classesID, 'schoolID' => $schoolID));
			...
			if(customCompute($student) && customCompute($classes)) {
				...
				$exams             = pluck($this->exam_m->get_exam(array('schoolID' => $schoolID)), 'exam', 'examID');
				$grades            = $this->grade_m->get_order_by_grade(array('schoolID' => $schoolID));
				$marks             = $this->mark_m->student_all_mark_array($queryArray);
				$markpercentages   = $this->markpercentage_m->get_order_by_markpercentage(array('schoolID' => $schoolID));
	...
	public function sectioncall() {
		...
		if((int)$id) {
			$schoolID = $this->session->userdata('schoolID');
			$allsection = $this->section_m->get_order_by_section(array("classesID" => $id, "schoolID" => $schoolID));
	...
	public function unique_comments() {
		foreach ($comments as $value) {
			if(strlen($value['value'] > 140) ) {
				$this->form_validation->set_message('unique_comments', 'Please maintain a 140 character limit.');
			}
		}
	}
