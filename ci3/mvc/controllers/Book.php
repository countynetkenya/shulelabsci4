...
*/
	...
	public function index() {
		$this->data['books'] = $this->book_m->get_order_by_book(array('schoolID' => $this->session->userdata('schoolID')));
	...
	public function edit() {
		...
		if((int)$id) {
			$this->data['book'] = $this->book_m->get_single_book(array('bookID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
			if($this->data['book']) {
				if($_POST) {
					$rules = $this->rules();
					$this->form_validation->set_rules($rules);
	...
	public function delete() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$book = $this->book_m->get_single_book(array('bookID' => $id, 'schoolID' => $this->session->userdata('schoolID')));
	...
	public function unique_book() {
		...
		$schoolID = $this->session->userdata('schoolID');
		if((int)$id) {
			$student = $this->book_m->get_order_by_book(array("book" => $this->input->post("book"), "bookID !=" => $id, "author" => $this->input->post('author'), "subject_code" => $this->input->post('subject_code'), 'schoolID' => $schoolID));
			...
			$student = $this->book_m->get_order_by_book(array("book" => $this->input->post("book"), "author" => $this->input->post('author'), "subject_code" => $this->input->post('subject_code'), 'schoolID' => $schoolID));
