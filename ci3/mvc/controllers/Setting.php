...
*/
    ...
    protected function rules()
    {
        $rules = [
            ...
            [
                'field' => 'fees_balance_notification',
                'label' => $this->lang->line("setting_fees_balance_notification"),
                'rules' => 'trim|required|xss_clean'
            ],
            [
            ...
            [
                'field' => 'inventory_allow_negative_stock',
                'label' => $this->lang->line('setting_inventory_allow_negative_stock'),
                'rules' => 'trim|required|xss_clean|in_list[0,1]'
            ]
        ];
    ...
    protected function superadmin_rules()
    {
        $rules = [
            [
                'field' => 'sname',
    ...
    public function index()
    {
        ...
        $schoolID 								  = $this->session->userdata('schoolID');
        $this->data['setting']      = $this->setting_m->get_setting($schoolID);
        $this->data['settingarray'] = $this->setting_m->get_setting_array($schoolID);
        $this->data['schoolyears']  = $this->schoolyear_m->get_order_by_schoolyear([ 'schooltype' => 'classbase', 'schoolID' => $schoolID ]);
        ...
        if ( $this->data['setting'] ) {
            if ( $_POST ) {
                ...
                $this->data['fees_balance_notification']         = $this->input->post('fees_balance_notification');
                ...
                if ( $this->form_validation->run() == false ) {
                    ...
                    if ( isset($array['language']['value']) ) {
                        $this->session->set_userdata('lang', $array['language']);
                    }

                    if ( isset($array['school_year']['value']) ) {
                    ...
                    $this->school_m->update_school(array('name' => $array['sname']), $schoolID);
                    $this->setting_m->insertorupdate($schoolID, $array);
    ...
    public function backendtheme()
    {
        ...
        $this->setting_m->update_setting(array('backend_theme' => $themeName), $this->session->userdata('schoolID'));
        echo $themeName;
    }
