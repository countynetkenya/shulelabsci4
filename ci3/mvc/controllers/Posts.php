...
class Posts extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['posts_categorys'] = pluck_multi_array($this->posts_category_m->get_posts_category(), 'posts_categoriesID', 'postsID');

        $this->data['posts_categories'] = pluck($this->posts_categories_m->get_posts_categories(), 'posts_categories', 'posts_categoriesID');
    ...
    public function add() {
        ...
        $this->data['media_gallerys_images'] = $this->media_gallery_m->get_media_gallery(array('media_gallery_type' => 1));

        $this->data['media_gallerys_audios'] = $this->media_gallery_m->get_media_gallery(array('media_gallery_type' => 2));

        $this->data['media_gallerys_videos'] = $this->media_gallery_m->get_media_gallery(array('media_gallery_type' => 3));
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(!empty($this->input->post('featured_image'))) {
                    $this->data['send_featured_image'] = $this->media_gallery_m->get_media_gallery($this->input->post('featured_image'));
                } else {
    ...
    public function edit() {
        ...
        if((int)$id) {
            $this->data['post'] = $this->posts_m->get_single_posts(array('postsID' => $id));
            if($this->data['post']) {
                ...
                if($_POST) {
                    ...
                    $this->data['send_featured_image'] = $this->media_gallery_m->get_media_gallery($this->data['post']->featured_image);
    ...
    public function delete() {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $this->data['posts'] = $this->posts_m->get_posts($id);
            if($this->data['posts']) {
    ...
    public function getFileInfo() {
        ...
        if((int) $id && $media_type) {
            $this->data['image_info'] = $this->media_gallery_m->get_media_gallery(array('media_galleryID' => $id));
    ...
    public function deleteFileInfo() {
        ...
        $status                         = false;
        if((int) $id) {
            $image_info = $this->media_gallery_m->get_media_gallery($id);
        ...
        if((int)$media_type && $allID && $ulclass_type) {
            ...
            if(customCompute($expID) >= 2) {
                ...
                foreach ($expID as $expKey => $expValue) {
                    if(!empty($expValue)) {
                        $get_media_gallery = $this->media_gallery_m->get_media_gallery($expValue);
                ...
                if((int)$allID) {
                    ...
                    $get_media_gallery = $this->media_gallery_m->get_media_gallery($allID);
