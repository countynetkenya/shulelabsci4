...
*/
	...
	protected function rules() {
		$rules = array(
			...
			array(
				'field' => 'classes_code',
				'label' => $this->lang->line("classes_code"),
				'rules' => 'trim|required|alpha_numeric|max_length[11]|xss_clean|callback_unique_classes_code'
			),
			array(
				'field' => 'divisionID',
				'label' => $this->lang->line("divisionID"),
				'rules' => 'trim|required|numeric|max_length[11]|xss_clean|callback_unique_division'
			),
			array(
				'field' => 'teacherID',
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$division = pluck($this->divisions_m->get_order_by_divisions(array("schoolID" => $schoolID)), 'divisions', 'divisionsID');
				$array = array(
					"classes" => $this->input->post("classes"),
					"classes_code" => $this->input->post("classes_code"),
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['teachers'] = $this->teacher_m->get_order_by_teacher(array("schoolID" => $schoolID));
			$this->data['divisions'] = $this->divisions_m->get_order_by_divisions(array("schoolID" => $schoolID));
			$this->data['classes'] = $this->classes_m->get_single_classes(array('classesID' => $id, 'schoolID' => $schoolID));
	...
	public function delete() {
		...
		if((int)$id) {
			$schoolID = $this->session->userdata('schoolID');
			$classes = $this->classes_m->get_single_classes(array('classesID' => $id, 'schoolID' => $schoolID));
	...
	public function unique_classes() {
		...
		if((int)$id) {
			$schoolID = $this->session->userdata('schoolID');
			$classes = $this->classes_m->get_order_by_classes(array("classes" => $this->input->post("classes"), "schoolID" => $schoolID, "classesID !=" => $id));
			if(customCompute($classes)) {
			...
			$classes = $this->classes_m->get_order_by_classes(array("classes" => $this->input->post("classes"), "schoolID" => $schoolID));
			if(customCompute($classes)) {
				$this->form_validation->set_message("unique_classes", "%s already exists");
				return FALSE;
			}
	...
	public function unique_classes_code() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$schoolID = $this->session->userdata('schoolID');
			$classes_code = $this->classes_m->get_order_by_classes(array("classes_code" => $this->input->post("classes_code"), "schoolID" => $schoolID, "classesID !=" => $id));
			if(customCompute($classes_code)) {
				$this->form_validation->set_message("unique_classes_code", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$classes_code = $this->classes_m->get_order_by_classes(array("classes_code" => $this->input->post("classes_code"), "schoolID" => $schoolID));
			if(customCompute($classes_code)) {
				$this->form_validation->set_message("unique_classes_code", "%s already exists");
				return FALSE;
			}
	...
	public function unique_division() {
		if($this->input->post('divisionID') == 0) {
			$this->form_validation->set_message("unique_division", "The %s field is required");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function unique_valid_number() {
		if($this->input->post('classes_code') < 0) {
