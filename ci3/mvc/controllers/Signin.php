...
*/
    public function __construct()
    {
        ...
        $this->session->set_userdata(array('lang' => $this->data["siteinfos"]->language));
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($signinManager['return']) {
                  $schoolID = explode(",", $this->data["user"]->schoolID);
                  ...
                  if(function_exists('is_super_admin') ? is_super_admin($sessionSnapshot) : ($usertypeID == 0 || ($usertypeID == 1 && $loginUserID == 1))) {
                    $siteInfo = $this->site_m->get_site(0);
                                $session = [
                                                "schoolID" => 0
                                ];
                  ...
                  } elseif($usertypeID == 1 || ($usertypeID >= 5 && $usertypeID <= 8)) {
                    if(customCompute($schoolID) > 1) {
                      redirect(base_url('school/select'));
                    } elseif(customCompute($schoolID) == 0) {
                      redirect(base_url('school/add'));
                    } else {
                      $siteInfo = $this->site_m->get_site($schoolID[0]);
                			$session = [
                					"schoolID" => $schoolID[0],
    ...
    private function _signInManager()
    {
        ...
        if ( isset($this->data['siteinfos']->captcha_status) && $this->data['siteinfos']->captcha_status == 0 ) {
