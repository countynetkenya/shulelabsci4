...
*/
	...
	public function photoupload() {
		...
		if((int)$id) {
			...
			$user = $this->parents_m->get_single_parents(array('parentsID' => $id, 'schoolID' => $schoolID));
		}
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$phoneArray = $this->input->post('phone');
				$countrycodeArray = $this->input->post('countrycode');
				for($i=0; $i<customCompute($phoneArray); $i++) {
					$phone .=  $countrycodeArray[$i] . $phoneArray[$i] . "/";
				}
				$phone = rtrim($phone, '/');
				$array['phone'] = $phone;
				...
				// For SMS
				$message = "Hi ". $this->input->post('name') .", your username '". $this->input->post('username') ."' and password '". $this->input->post('password') ."'. Get the Shulelabs mobile app at ". site_url("app") ." or visit us on web ". site_url();
				/*$result = $this->allgetway_send_message("smsleopard", $this->input->post('phone'), $message);
				if($result['check']) {
					$array = array(
						'usertypeID' => '0',
						'users' => $this->input->post('othersms_name'),
						'type' => ucfirst($this->lang->line('mailandsms_othersms')),
						'mes/sage' => $this->input->post('othersms_message'),
						'year' => date('Y'),
				...
				redirect(base_url("parents/view/".$parentID));
			}
		} else {
			$this->data["subview"] = "parents/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if ((int)$id) {
			$this->data['parents'] = $this->parents_m->get_single_parents(array('parentsID' => $id, 'schoolID' => $schoolID));
			if($this->data['parents']) {
				if($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$array["modify_date"] = date("Y-m-d h:i:s");
						$array['photo'] = $this->upload_data['file']['file_name'];
						$array['phone'] = implode("/", $this->input->post('phone'));
	...
	private function getView($parentsID) {
		$schoolID = $this->session->userdata('schoolID');
		if ((int)$parentsID) {
			$parents = $this->parents_m->get_single_parents(array('parentsID' => $parentsID, 'schoolID' => $schoolID));
			$this->plucInfo();
	...
	private function childrenInfo($parents) {
		...
		if(customCompute($parents)) {
			...
			$this->data['childrens'] = $this->studentrelation_m->general_get_order_by_student(array('parentID' => $parents->parentsID, 'srschoolyearID' => $schoolyearID, 'srschoolID' => $schoolID));
		}
	}
	...
	private function plucInfo() {
		...
		$this->data['classes'] = pluck($this->classes_m->general_get_classes(array('schoolID' => $schoolID)),'classes','classesID');
	...
	public function download_document() {
		...
		if((int)$documentID && (int)$parentsID) {
			if((permissionChecker('parents_add') && permissionChecker('parents_delete')) || ($this->session->userdata('usertypeID') == 4 && $this->session->userdata('loginuserID') == $parentsID)) {
				$document = $this->document_m->get_single_document(array('documentID' => $documentID, 'schoolID' => $schoolID));
	...
	public function print_preview() {
		if(permissionChecker('parents_view') || (($this->session->userdata('usertypeID') == 4) && permissionChecker('parents') && ($this->session->userdata('loginuserID') == htmlentities(escapeString($this->uri->segment(3)))))) {
			...
			$schoolID  = $this->session->userdata('schoolID');
			if((int)$parentsID) {
				$this->data['parents'] = $this->parents_m->get_single_parents(array('parentsID' => $parentsID, 'schoolID' => $schoolID));
	...
	public function unique_email() {
		if($this->input->post('email')) {
			...
			if((int)$id) {
				$schoolID = $this->session->userdata('schoolID');
				$parents = $this->parents_m->get_single_parents(array('parentsID' => $id, 'schoolID' => $schoolID));
	...
	public function active() {
		if(permissionChecker('parents_edit')) {
			...
			if($id != '' && $status != '') {
				if((int)$id) {
					$schoolID = $this->session->userdata('schoolID');
					$parent = $this->parents_m->get_single_parents(array('parentsID' => $id, 'schoolID' => $schoolID));
