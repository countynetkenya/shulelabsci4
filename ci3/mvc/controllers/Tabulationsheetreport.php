		...
		if(permissionChecker('tabulationsheetreport')) {
			...
			if($_POST) {
			    ...
			    exit;
					...
					$schoolID 		= $this->session->userdata('schoolID');
					$studentQuery = ['srclassesID' => $classesID, 'srschoolID' => $schoolID];
					if($sectionID > 0) {
					...
					$this->data['mandatorysubjects'] = $this->subject_m->general_get_order_by_subject(array('classesID'=>$classesID, 'type' => 1, 'schoolID' => $schoolID));
					...
					$marks                           = $this->mark_m->get_order_by_all_student_mark_with_markrelation(['schoolID' => $schoolID, 'schoolyearID' => $schoolyearID, 'classesID' => $classesID, 'examID' => $examID]);
