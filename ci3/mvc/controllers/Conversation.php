...
class Conversation extends Admin_Controller
{
    ...
    */
    ...
    public function index()
    {
        ...
        $this->data['designation'] = pluck($this->teacher_m->general_get_order_by_teacher(array('schoolID' => $this->session->userdata("schoolID"))), 'designation', 'teacherID');
    ...
    public function draft_send( $id )
    {
        if ( (int) $id ) {
            ...
            $conversation = $this->conversation_m->get_single_conversation(array('id' => $id, 'schoolID' => $schoolID));
    ...
    public function create()
    {
        ...
        $this->data['usertypes']    = $this->conversation_m->get_usertype_by_permission(array('permission_relationships.schoolID' => $schoolID));
        $this->data['classes']      = $this->classes_m->get_order_by_classes(array('schoolID' => $schoolID));
        if(permissionChecker('conversation_add')) {
          if ( $_POST ) {
              $rules = $this->rules();
    ...
    {
        $schoolID = $this->session->userdata("schoolID");
        $classes  = $this->conversation_m->get_order_by_classes(array('schoolID' => $schoolID));
    ...
    {
        $schoolID = $this->session->userdata("schoolID");
        $admins   = $this->systemadmin_m->get_order_by_systemadmin(array('schoolID' => $schoolID));
    ...
    public function userCall()
    {
        ...
        $schoolID   = $this->session->userdata("schoolID");
        $users = $this->user_m->get_order_by_user([ 'usertypeID' => $userTypeID , 'schoolID' => $schoolID ]);
    ...
    private function _currentUser( $userGroupID )
    {
        ...
        if ( $userGroupID == 1 ) {
            if ( !$this->input->post('systemadminID') ) {
                $users = $this->systemadmin_m->get_order_by_systemadmin(array('schoolID' => $schoolID));
            } else {
                $systemadminID = $this->input->post('systemadminID');
                $users         = $this->systemadmin_m->get_order_by_systemadmin([ 'systemadminID' => $systemadminID, 'schoolID' => $schoolID ]);
            }
        } elseif ( $userGroupID == 2 ) {
            if ( !$this->input->post('teacherID') ) {
                $users = $this->teacher_m->get_order_by_teacher(array('schoolID' => $schoolID));
            } else {
                $teacherID = $this->input->post('teacherID');
                $users     = $this->teacher_m->get_order_by_teacher([ 'teacherID' => $teacherID, 'schoolID' => $schoolID ]);
            }
        } elseif ( $userGroupID == 3 ) {
            $schoolYearID = $this->session->userdata('defaultschoolyearID');
            if ( !$this->input->post('classID') ) {
                $users = $this->studentrelation_m->get_order_by_student([ 'srschoolyearID' => $schoolYearID, 'srschoolID' => $schoolID ]);
            } else {
            ...
            if ( !$this->input->post('parentID') ) {
                ...
                $users     = $this->parents_m->get_order_by_parents([ 'parentsID' => $parentsID, 'schoolID' => $schoolID ]);
            }
        } else {
            if ( !$this->input->post('userID') ) {
                $users = $this->user_m->get_order_by_user([ 'usertypeID' => $userGroupID, 'schoolID' => $schoolID ]);
            } else {
                $userID = $this->input->post('userID');
                $users  = $this->user_m->get_order_by_user([ 'userID' => $userID, 'schoolID' => $schoolID ]);
            }
        }
        return $users;
    }
