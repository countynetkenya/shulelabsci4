...
class Stock extends Admin_Controller
{
    ...
    */
    ...
    function __construct()
    {
        ...
        $this->lang->load('stock', $language);
    }

    protected function rules_adjustment()
    {
        $rules = [
            [
            ...
            [
                ...
                'rules' => 'trim|xss_clean|required|callback_unique_productitems|callback_check_sufficient_stock_for_adjustment'
            ],
        ];
        return $rules;
    }

    protected function rules_movement()
    {
        $rules = [
            [
                'field' => 'fromproductwarehouseID',
            ...
            [
                'field' => 'productitems',
                'label' => $this->lang->line("stock_productitem"),
                'rules' => 'trim|xss_clean|required|callback_unique_productitems|callback_check_sufficient_stock_for_movement'
            ],
        ];
    ...
    public function check_sufficient_stock_for_adjustment()
    {
        $productwarehouseID = $this->input->post('fromproductwarehouseID');
        if($productwarehouseID > 0) {
            $productitems = json_decode($this->input->post('productitems'));
            if(customCompute($productitems)) {
                foreach ($productitems as $productitem) {
                    if($productitem->amount < 0) {
                        $quantity_to_decrease = abs($productitem->amount);
                        if(!has_sufficient_stock($productitem->productID, $productwarehouseID, $quantity_to_decrease)) {
                            $product = $this->product_m->get_single_product(['productID' => $productitem->productID]);
                            $this->form_validation->set_message("check_sufficient_stock_for_adjustment", "The ". $product->productname." is stock out.");
                            return FALSE;
                        }
                    }
                }
    ...
    public function check_sufficient_stock_for_movement()
    {
        $productwarehouseID = $this->input->post('fromproductwarehouseID');
        if($productwarehouseID > 0) {
            $productitems = json_decode($this->input->post('productitems'));
            if(customCompute($productitems)) {
                foreach ($productitems as $productitem) {
                    $quantity_to_decrease = $productitem->amount;
                    if(!has_sufficient_stock($productitem->productID, $productwarehouseID, $quantity_to_decrease)) {
                        $product = $this->product_m->get_single_product(['productID' => $productitem->productID]);
                        $this->form_validation->set_message("check_sufficient_stock_for_movement", "The ". $product->productname." is stock out.");
                        return FALSE;
                    }
                }
            }
	  ...
	  public function index()
    {
        ...
        $this->data['mainstocks']        = $this->mainstock_m->get_order_by_mainstock(array('schoolID' => $schoolID));
    ...
    public function adjust() {
        ...
        $this->data["subview"] = "stock/adjust";
        $this->load->view('_layout_main', $this->data);
    }
    ...
    public function saveadjustment()
    {
        ...
        if(permissionChecker('stock_adjust')) {
            if($_POST) {
                $rules = $this->rules_adjustment();
    ...
    {
        $retArray['status'] = FALSE;
        if(permissionChecker('stock_move')) {
            if($_POST) {
                $rules = $this->rules_movement();
                $this->form_validation->set_rules($rules);
                if ($this->form_validation->run() == FALSE) {
                    $retArray['error']  = $this->form_validation->error_array();
                    $retArray['status'] = FALSE;
                    ...
                    $stockMainArray = [
                        'stockfromwarehouseID'    => $fromproductwarehouseID,
                        'stocktowarehouseID'      => $toproductwarehouseID,
                        'type'                    => "movement",
                        'memo'                    => $memo,
                        'schoolID'                => $schoolID,
                        'mainstockuserID'         => $this->session->userdata('loginuserID'),
