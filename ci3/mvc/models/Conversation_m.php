...
class conversation_m extends MY_Model
{
    ...
    public function get_single_conversation($array=NULL)
    {
    ...
    public function get_usertype_by_permission($array)
    {
        $usertypeID = $this->session->userdata("usertypeID");
    ...
    {
        $classes = new Classes_m;
        $teacherclass = new Teacherclasses_m;
    		$usertypeID = $this->session->userdata('usertypeID');
        $userID = $this->session->userdata('userID');
        $teacher = $teacherclass->get_single_teacher_class(array('teacherID' => $userID));
        $designation = strtolower($teacher->designation);
    		if($usertypeID == 2) {
          if($designation == "class teacher")
    	       return $teacherclass->get_order_by_teacher_class($array);
          elseif($designation == "principal" || $designation == "discipline assigned" || $designation == "accountant")
    	       return $classes->general_get_order_by_classes($array);
    	    ...
    	    return $studentparentclasses->get_order_by_studentparent_class($array);
    			...
    			$query = $classes->general_get_order_by_classes($array);
    			return $query;
    		}
  	}
}
