...
class Subject_m extends MY_Model {
	...
	protected $_order_by = "report_order asc";
	...
	public function get_join_subject($id) {
	    ...
	    return $studentsubject->get_subject_with_class($id);
			...
			$this->db->order_by('subject.report_order asc');
			$query = $this->db->get();
			return $query->result();
		}
	}
	...
	public function get_order_by_subject_with_subjectteacher($array=NULL) {
		$array = $this->prefixLoad($array);
		$this->db->select('*');
		$this->db->from('subject');
