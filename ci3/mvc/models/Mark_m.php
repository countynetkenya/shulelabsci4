...
class Mark_m extends MY_Model {
	...
	function update_batch_mark($data, $id = NULL) {
	...
	public function student_all_mark_array($array) {
		...
		if(isset($array['schoolID'])) {
			$this->db->where('mark.schoolID', $array['schoolID']);
		}
	...
	public function get_order_by_all_student_mark_with_markrelation2($markArray) {
		$this->db->select('mark.*, markrelation.markrelationID, markrelation.markpercentageID, markrelation.mark, subjectteacher.teacherID');
		$this->db->from('mark');
		$this->db->join('markrelation', 'markrelation.markID = mark.markID', 'LEFT');
		$this->db->join('subjectteacher', 'mark.subjectID = subjectteacher.subjectID', 'LEFT');
		if(isset($markArray['schoolID'])) {
			$this->db->where('mark.schoolID', $markArray['schoolID']);
		}
		...
		if(isset($markArray['examID'])) {
			$this->db->where_in('mark.examID', $markArray['examID']);
		}
