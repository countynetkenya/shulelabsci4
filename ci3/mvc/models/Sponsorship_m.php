...
class Sponsorship_m extends MY_Model
{
    ...
    public function get_sponsorship_with_student($array=[])
    {
    ...
    public function get_sponsorship_for_report($array)
    {
        if ($array['id'] == 1) {
            ...
            $this->db->where('sponsorship.schoolID', $array['schoolID']);
            $this->db->order_by('sponsorship.sponsorshipID desc');
            $query = $this->db->get();
            return $query->result();
        } else if ($array['id'] == 2) {
