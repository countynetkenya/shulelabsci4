...
class Ebooks_m extends MY_Model {
	...
	public function get_order_by_ebooks_with_authority($classes, $schoolID) {
		$this->db->select('*');
		$this->db->from('ebooks');
	...
	public function get_single_ebooks_with_authority($classes, $ebooksID, $schoolID) {
		$this->db->select('*');
		$this->db->from('ebooks');
		$this->db->group_start();
		$this->db->where('authority', '0');
	...
	public function get_order_by_ebooks_with_authority_pagination($classes, $schoolID, $limit, $offset) {
		$this->db->select('*');
		$this->db->from('ebooks');
		$this->db->where('authority', '0');
		$this->db->where('schoolID', $schoolID);
