...
class Payment_m extends MY_Model {
	...
	public function get_payment_with_student($array = null) {
		$array = $this->prefixLoad($array);
		$this->db->select('*');
		$this->db->from('payment');
		$this->db->join('student', 'student.studentID = payment.studentID', 'LEFT');
		$this->db->where($array);
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function get_payment_with_studentrelation($array=NULL) {
		$this->db->select('payment.*, studentrelation.*');
		$this->db->from('payment');
		$this->db->join('studentrelation', 'studentrelation.srstudentID = payment.studentID AND studentrelation.srschoolyearID = payment.schoolyearID', 'LEFT');
		$this->db->where($array);
		$this->db->order_by($this->_order_by);
	...
	public function get_payment_with_studentrelation_by_studentID_and_schoolyearID($studentID, $schoolyearID) {
		...
		$this->db->select('payment.*, weaverandfine.weaver, weaverandfine.fine');
		$this->db->from('payment');
		//$this->db->join('invoice', 'invoice.invoiceID = payment.invoiceID', 'LEFT');
	...
	function get_payment_wherein($array, $array2) {
	...
	public function get_single_payment_with_studentrelation($array=NULL) {
		$this->db->select('payment.*, studentrelation.*');
		$this->db->from('payment');
		$this->db->join('studentrelation', 'studentrelation.srstudentID = payment.studentID AND studentrelation.srschoolyearID = payment.schoolyearID', 'LEFT');
		$this->db->where($array);
	...
	public function get_all_payment_for_report($queryArray) {
		$this->db->select('payment.*, globalpayment.classesID, division, srstudentID, srname, srclasses, group');
		$this->db->from('payment');
		$this->db->join('globalpayment', 'payment.globalpaymentID = globalpayment.globalpaymentID','LEFT');
		$this->db->join('classes', 'globalpayment.classesID = classes.classesID', 'LEFT');
		$this->db->join('studentrelation', 'payment.studentID = studentrelation.srstudentID', 'LEFT');
		...
		if((isset($queryArray['schoolID']) && $queryArray['schoolID'] != 0) || (isset($queryArray['schoolyearID']) && $queryArray['schoolyearID'] != 0) || (isset($queryArray['classesID']) && $queryArray['classesID'] != 0) || (isset($queryArray['sectionID']) && $queryArray['sectionID'] != 0) || (isset($queryArray['studentID']) && $queryArray['studentID'] != 0)) {
			if(isset($queryArray['schoolID']) && $queryArray['schoolID'] != 0) {
				...
				$this->db->where('payment.schoolID', $queryArray['schoolID']);
			}
		...
		if(isset($queryArray['termFrom']) && $queryArray['termFrom'] != "") {
			$this->db->where('paymentdate >=', $queryArray['termFrom']);
		}

		if(isset($queryArray['termTo']) && $queryArray['termTo'] != "") {
			$this->db->where('paymentdate >=', $queryArray['termTo']);
		}
