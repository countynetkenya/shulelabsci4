...
class Maininvoice_m extends MY_Model {
	...
	public function get_maininvoice_with_studentrelation($array) {
		$this->db->select('*');
		...
		$this->db->where('maininvoice.schoolID', $array['schoolID']);
		$this->db->where('studentrelation.srschoolID', $array['schoolID']);
		if($array['schoolyearID'] != NULL) {
			$this->db->where('maininvoice.maininvoiceschoolyearID', $array['schoolyearID']);
	...
	public function get_maininvoice_with_studentrelation_by_maininvoiceID($array) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
		$this->db->join('studentrelation', 'studentrelation.srstudentID = maininvoice.maininvoicestudentID AND studentrelation.srschoolyearID = maininvoice.maininvoiceschoolyearID', 'LEFT');
		$this->db->where('maininvoice.maininvoiceID', $array['invoiceID']);
		$this->db->where('maininvoice.maininvoicedeleted_at', 1);
