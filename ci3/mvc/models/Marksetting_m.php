...
class Marksetting_m extends MY_Model {
	...
	public function get_exam($array = null) {
		$array = $this->prefixLoad($array);
		$marktypeID = $array['marktypeID'];
		if($marktypeID == 4) {
			return $this->exam_m->get_order_by_exam(array('schoolID' => $array['schoolID']));
  ...
  public function get_exam_with_class( $classesID = 0 )
  {
      ...
      if ( $marktypeID == 4 ) {
          $exams = $this->exam_m->get_order_by_exam(array('schoolID' => $schoolID));
      } elseif ( ( $marktypeID == 5 ) || ( $marktypeID == 6 ) ) {
          if ( (int) $classesID ) {
              ...
              $this->db->where('marksetting.classesID', $classesID);
							$this->db->where('marksetting.schoolID', $schoolID);
              $query = $this->db->get();
              $exams =  $query->result();
          }
      } else {
	...
	public function get_marksetting_markpercentages_add($array) {
		...
		$queryArray['schoolID']     = (int)$schoolID;
	...
	public function get_marksetting_markpercentages() {
		...
		$classes    = $this->classes_m->get_order_by_classes(['classesID !='=> $exclassID, 'schoolID' => $schoolID]);
		$exams      = $this->exam_m->get_order_by_exam(['schoolID' => $schoolID]);
		$subjects   = pluck_multi_array($this->subject_m->get_order_by_subject(array('schoolID' => $schoolID)), 'obj', 'classesID');
