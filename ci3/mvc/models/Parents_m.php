...
class Parents_m extends MY_Model {
	...
	public function get_parents2($array=NULL) {
    $this->db->select('*');
    $this->db->from('parents');
    $this->db->join('student', 'student.parentID = parents.parentsID', 'LEFT');
	...
	function get_parents_wherein($array) {
        $query = $this->db->select('*')->from($this->_table_name)->where_in('parentsID', $array)->get();
        return $query->result();
    }
