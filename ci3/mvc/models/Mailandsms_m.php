...
class Mailandsms_m extends MY_Model {
	...
	function get_mailandsms_with_usertypeID($queryArray) {
		...
		if(isset($queryArray['reviewed'])) {
			$this->db->where('reviewed', $queryArray['reviewed']);
		}
		...
		if(isset($queryArray['schoolID'])) {
			$this->db->where('mailandsms.schoolID', $queryArray['schoolID']);
		}
