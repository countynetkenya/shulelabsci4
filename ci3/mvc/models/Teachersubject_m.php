...
class Teachersubject_m extends CI_Model
{
    ...
    protected $_primary_key    = 'subject.subjectID';
  		...
  		if(is_array($array)) {
  			if(customCompute($array)) {
  				foreach ($array as $arkey =>  $ar) {
  					if($arkey == "classesID" || $arkey == "schoolID") {
  						unset($array[$arkey]);
  						$array['subject.'.$arkey] = $ar;
  					}
  				}
  			}
  		}
    ...
    public function teacherSubject()
    {
        ...
        return mergeArray($classSectionResult, $routineResult, 'subjectID');
    }
    ...
    public function get_single_teacher_subject($array = null)
    {
        if (is_array($array)) {
            if (isset($array['subjectID'])) {
                ...
                return $query;
            } else {$this->db->where_in($this->_primary_key, $subjectArray);
                $this->db->where($array);
                $this->db->order_by($this->_order_by);
                $query = $this->db->get($this->_table_name);
                return $query->row();
    ...
    public function get_order_by_teacher_subject($array = null)
    {
        ...
        if (is_array($array)) {
          if (isset($array['subjectID'])) {
              ...
              $this->db->where($array);
		          $this->db->join('classes', 'classes.classesID = subject.classesID', 'LEFT');
              $this->db->order_by('subject.classesID');
              $query = $this->db->get($this->_table_name);
              $query = $query->result();
              return $query;
          }
