<?php
    ...
    function btn_resend( $uri, $name )
    {
        if ( visibleButton($uri) ) {
            return anchor($uri, "<i class='fa fa-repeat'></i>",
                "class='btn btn-success btn-xs mrg' data-placement='top' data-toggle='tooltip' data-original-title='" . $name . "'");
        }
    ...
    function menu_permission_candidates($node)
    {
        if (!is_array($node)) {
            return [];
        }
        ...
        foreach (['permission', 'permission_key'] as $field) {
            if (!empty($node[$field]) && is_string($node[$field])) {
                $candidates[] = trim($node[$field]);
            }
        }
    ...
    function pluck_multi_values( $array, $values, $key = null )
    {
        $returnArray = [];
        if ( customCompute($array) ) {
            foreach ( $array as $item ) {
              foreach ($values as $value) {
                if ( $key != null ) {
                    $returnArray[ $item->$key ][$value] = strtolower($value) == 'obj' ? $item : $item->$value;
                } else {
                    if ( $value == 'obj' ) {
                        $returnArray[] = $item;
                    } else {
    ...
    { /* DD OK */
        ...
        $systemadmin = $CI->systemadmin_m->get_order_by_systemadmin(array('schoolID' => $arrays['schoolID']));
        if ( customCompute($systemadmin) ) {
            $returnArray[1] = pluck($systemadmin, 'obj', 'systemadminID');
        }

        $teacher = $CI->teacher_m->general_get_order_by_teacher(array('schoolID' => $arrays['schoolID']));
        ...
        $parent = $CI->parents_m->get_order_by_parents(array('schoolID' => $arrays['schoolID']));
        if ( customCompute($parent) ) {
            $returnArray[4] = pluck($parent, 'obj', 'parentsID');
        }

        $users = $CI->user_m->get_order_by_user(array('schoolID' => $arrays['schoolID']));
    ...
    { /* DD OK */
        ...
        $schoolID    = $CI->session->userdata('schoolID');
        $returnArray = [];

        $systemadmin = $CI->systemadmin_m->get_order_by_systemadmin(array('schoolID' => $schoolID));
        if ( customCompute($systemadmin) ) {
            $returnArray[1] = pluck($systemadmin, 'obj', 'systemadminID');
        }

        $teacher = $CI->teacher_m->general_get_order_by_teacher(array('schoolID' => $schoolID));
        if ( customCompute($teacher) ) {
            $returnArray[2] = pluck($teacher, 'obj', 'teacherID');
        }

        $users = $CI->user_m->get_order_by_user(array('schoolID' => $schoolID));
        if ( customCompute($users) ) {
            foreach ( $users as $user ) {
                $returnArray[ $user->usertypeID ][ $user->userID ] = $user;
            }
    ...
    function display_menu( $nodes, &$menu )
    {
        ...
        foreach ( $nodes as $key => $node ) {
            ...
            $skipPermission = !empty($node['skip_permission']);

            $hasPermission = $skipPermission;

            if (!$hasPermission) {
                $permissionCandidates = menu_permission_candidates($node);
