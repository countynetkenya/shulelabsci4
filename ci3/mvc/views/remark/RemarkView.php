<div class="box">
    <div class="box-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"> <?=$this->lang->line('menu_remark')?></li>
        </ol>
    </div><!-- /.box-header -->
    <!-- form start -->
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                <div class="form-group col-sm-4" id="classesDiv">
                    <label><?=$this->lang->line("remark_class")?><span class="text-red"> * </span></label>
                    <?php
                        $classesArray = array(
                            "0" => $this->lang->line("remark_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="examDiv">
                    <label><?=$this->lang->line("remark_exam")?><span class="text-red"> * </span></label>
                    <?php
                        $examArray = array(
                            "0" => $this->lang->line("remark_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="sectionDiv">
                    <label><?=$this->lang->line("remark_section")?></label>
                    <?php
                        $sectionArray = array(
                            "0" => $this->lang->line("remark_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="studentDiv">
                    <label><?=$this->lang->line("remark_student")?></label>
                    <?php
                        $studentArray = array(
                            "0" => $this->lang->line("remark_please_select"),
                        );
                ...
                <div class="col-sm-4">
                    <button id="get_tabulationsheetreport" class="btn btn-success" style="margin-top:23px;"> <?=$this->lang->line("remark_submit")?></button>
                </div>
            </div>
        </div><!-- row -->
    </div><!-- Body -->
...
<script type="text/javascript">
    ...
    $(document).on('change', "#classesID", function() {
        ...
        if(classesID == '0'){
            ...
            $('#sectionDiv').hide('slow');
            $('#studentDiv').hide('slow');
            $('#examID').html('<option value="0">'+"<?=$this->lang->line("remark_please_select")?>"+'</option>');
            $('#sectionID').html('<option value="0">'+"<?=$this->lang->line("remark_please_select")?>"+'</option>');
            $('#studentID').html('<option value="0">'+"<?=$this->lang->line("remark_please_select")?>"+'</option>');
        } else {
            $('#examDiv').show('slow');
            $('#sectionDiv').show('slow');
            $.ajax({
                type: 'POST',
                url: "<?=base_url('remark/getExam')?>",
            ...
            $.ajax({
                type: 'POST',
                url: "<?=base_url('remark/getSection')?>",
                data: {"classesID" : classesID},
                dataType: "html",
                success: function(data) {
                   $('#sectionID').html(data);
    ...
    $(document).on('change',"#sectionID", function() {
        ...
        if(sectionID == '0') {
            ...
            $.ajax({
                type: 'POST',
                url: "<?=base_url('remark/getStudent')?>",
    ...
    function ajaxCall(passData) {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('remark/getTabulatonsheetReport')?>",
            data: passData,
            dataType: "html",
            success: function(data) {
