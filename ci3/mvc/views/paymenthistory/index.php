        ...
        <div class="row">
            <div class="col-sm-12">
				...
				<?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                    <?php if(permissionChecker('payment')) { ?>
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                        <label for="date from" class="control-label">
                            <?=$this->lang->line('paymenthistory_from')?>
                        </label>
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                        <label for="date to" class="control-label">
                            <?=$this->lang->line('paymenthistory_to')?>
                        </label>
                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
                                ...
                                <th><?=$this->lang->line('paymenthistory_transaction')?></th>
                                <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                            ...
                            <?php if(customCompute($payments)) {$i = 1; foreach($payments as $payment) { if($payment->paymentamount != '') { ?>
                                <tr>
                                    <td data-title="<?=$this->lang->line('slno')?>">
                                        <?php echo $payment->globalpaymentID; ?>
                                    </td>
                                    ...
                                    <td data-title="<?=$this->lang->line('paymenthistory_transaction')?>">
                                        <?php echo $payment->transactionID;  ?>
                                    </td>
                                    <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
										<td data-title="<?=$this->lang->line('action')?>">
											<?php echo btn_view('paymenthistory/view/'.$payment->paymentID, $this->lang->line('view')) ?>
											<?php if((permissionChecker('paymenthistory_edit') || permissionChecker('paymenthistory_delete')) && $payment->paymenttype != 'Paypal' && $payment->paymenttype != 'Stripe' && $payment->paymenttype != 'PayUmoney') { ?>
