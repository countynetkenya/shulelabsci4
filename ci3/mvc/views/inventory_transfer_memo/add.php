...
<div class="row">
    <div class="col-sm-3">
      <div class="box">
          ...
          <div class="box-body">
              <form role="form" method="post" enctype="multipart/form-data" id="stockMoveDataForm">

                <div class="fromproductwarehouseDiv form-group <?=form_error('fromproductwarehouseID') ? 'has-error' : '' ?>" >
                    <label for="fromproductwarehouseID">
                        <?=$this->lang->line("stock_from")?> <span class="text-red">*</span>
                    </label>
                    <?php
                ...
                <div class="toproductwarehouseDiv form-group <?=form_error('toproductwarehouseID') ? 'has-error' : '' ?>" >
                    <label for="toproductwarehouseID">
                        <?=$this->lang->line("stock_to")?> <span class="text-red">*</span>
                    </label>
                    <?php
                        $productwarehouseArray = array(0 => $this->lang->line("stock_select_warehouse"));
                        if(customCompute($productwarehouses)) {
                        ...
                        echo form_dropdown("toproductwarehouseID", $productwarehouseArray, set_value("toproductwarehouseID"), "id='toproductwarehouseID' class='form-control select2'");
                    ?>
                    <span class="text-red">
                        <?php echo form_error('toproductwarehouseID'); ?>
                    </span>
                </div>
                ...
                <div class="form-group">
                    <input id="moveStockButton" type="button" class="btn btn-success" value="<?=$this->lang->line("move_title")?>" >
                </div>
              </form>
          </div>
      </div>
        ...
        <div class="box">
            <div class="box-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?=$this->lang->line('menu_add')?> <?=$this->lang->line('menu_inventory_transfer_memo')?></li>
                </ol>
            </div><!-- /.box-header -->
            <div class="box-body">
...
<script type="text/javascript">
    ...
    function productItemDesign(productID, productText) {
        ...
        var permission = <?= (permissionChecker('stock_move') != 1) ? 0 : 1; ?>;
        ...
        var text = '<tr id="tr_'+randID+'" moveproductID="'+productID+'">';
            text += '<td>';
                text += lastTdNumber;
            text += '</td>';
    ...
    $(document).on('keyup', '.change-amount', function() {
        ...
        var productID = tr.attr('moveproductID');
    ...
    $(document).on('click', '.deleteBtn', function(er) {
      ...
      $(document).on('click', '#moveStockButton', function() {
            var error=0;
            var field = {
                'fromproductwarehouseID'                 : $('#fromproductwarehouseID').val(),
                'toproductwarehouseID'                   : $('#toproductwarehouseID').val(),
                'memo'                                  : $('#memo').val(),
            };

            if(field['fromproductwarehouseID'] === '0') {
                $('.fromproductwarehouseDiv').addClass('has-error');
                error++;
            } else {
                $('.fromproductwarehouseDiv').removeClass('has-error');
            }

            if(field['toproductwarehouseID'] === '0') {
                $('.toproductwarehouseDiv').addClass('has-error');
                error++;
            } else {
                $('.toproductwarehouseDiv').removeClass('has-error');
            }

            if(field['toproductwarehouseID'] === field['fromproductwarehouseID']) {
                $('.toproductwarehouseDiv').addClass('has-error');
                error++;
            } else if(field['toproductwarehouseID'] !== '0') {
                $('.toproductwarehouseDiv').removeClass('has-error');
            }

            var productitems = $('tr[id^=tr_]').map(function(){
                return { productID : $(this).attr('moveproductID'), amount: $(this).children().eq(3).children().val()};
            }).get();
            ...
            if(error === 0) {
                ...
                var formData = new FormData($('#stockMoveDataForm')[0]);
        ...
        function ajaxCall(passData) {
            $.ajax({
                type: 'POST',
                url: "<?=base_url('inventory_transfer_memo/savemovement')?>",
                data: passData,
                async: true,
                dataType: "html",
        ...
        function errrorLoader(response) {
            if(response.status) {
                window.location = "<?=base_url("inventory_transfer_memo/index")?>";
            } else {
                $('#moveStockButton').removeAttr('disabled');
