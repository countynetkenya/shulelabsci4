                ...
                <form role="form" method="post" enctype="multipart/form-data" id="creditmemoDataForm">
                    ...
                    <div class="classesDiv form-group <?=form_error('classesID') ? 'has-error' : '' ?>" >
                        <label for="classesID">
                            ...
                            <?php
                                ...
                                echo form_dropdown("classesID", $classesArray, set_value("classesID", $maincreditmemo->maincreditmemoclassesID), "id='classesID' class='form-control select2'");
                            ?>
                        <span class="text-red">
                    ...
                    <div class="studentDiv form-group <?=form_error('studentID') ? 'has-error' : '' ?>" >
                        <label for="studentID">
                            ...
                            <?php
                                $studentArray = array('0' => $this->lang->line("creditmemo_select_student"));
                                if(customCompute($students)) {
                                    foreach ($students as $student) {
                                        $studentArray[$student->srstudentID] = $student->srname." - ".$this->lang->line('creditmemo_roll')." - ".$student->srstudentID;
                                    }
                                }
                                echo form_dropdown("studentID", $studentArray, set_value("studentID", $maincreditmemo->maincreditmemostudentID), "id='studentID' class='form-control select2'");
                            ?>
                        <span class="text-red">
					...
					<div class="form-group <?=form_error('schooltermID') ? 'has-error' : '' ?>" >
                        <label for="schooltermID">
                            ...
                            <?php
                                ...
                                echo form_dropdown("schooltermID", $termsArray, set_value("schooltermID", $maincreditmemo->maincreditmemoschooltermID), "id='schooltermID' class='form-control select2'");
                            ?>
                        <span class="text-red">
                    ...
                    <div class="dateDiv form-group <?=form_error('date') ? 'has-error' : '' ?>" >
                        <label for="date">
                            ...
                            <input type="text" class="form-control" id="date" name="date" value="<?=set_value('date', date('d-m-Y', strtotime($maincreditmemo->maincreditmemodate)))?>" >
                        <span class="text-red">
					...
					<div class="form-group">
						...
						<textarea id="memo" name="memo" class="form-control"><?=$maincreditmemo->maincreditmemomemo?></textarea>
					</div>

                    <input id="editCreditMemoButton" type="button" class="btn btn-success" value="<?=$this->lang->line("update_creditmemo")?>" >
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="box">
            <div class="box-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?=$this->lang->line('menu_edit')?> <?=$this->lang->line('menu_creditmemo')?></li>
                </ol>
            </div><!-- /.box-header -->
            <div class="box-body">
                            ...
                            <?php
                                if(customCompute($creditmemos)) {
                                    ...
                                    foreach ($creditmemos as $creditmemo) {
                                        ...
                                        $totalAmount += $creditmemo->amount;

                                        echo '<tr id="tr_'.$randID.'" creditmemocredittypeID="'.$creditmemo->credittypeID.'">';
                                            echo '<td>';
                                                echo $i;
                                            echo '</td>';

                                            echo '<td>';
                                                echo isset($credittypes[$creditmemo->credittypeID]) ? $credittypes[$creditmemo->credittypeID]->credittypes  : '';
                                            echo '</td>';

                                            echo '<td>';
                                                echo '<input type="text" class="form-control change-amount" id="td_amount_id_'.$randID.'" data-amount-id="'.$randID.'" value="'.$creditmemo->amount.'">';
                                            echo '</td>';

                                            echo '<td>';
                                                echo '<a style="margin-top:3px" href="#" class="btn btn-danger btn-sm deleteBtn" id="credittype_'.$randID.'" data-credittype-id="'.$randID.'"><i class="fa fa-trash-o"></i></a>';
                                            echo '</td>';
                                        echo '</tr>';
                                        $i++;
                                    }
...
<script type="text/javascript">
    ...
    $(document).on('click', '#editCreditMemoButton', function() {
        var error=0;;
        var field = {
        ...
        if(error === 0) {
            ...
            formData.append("editID", <?=$maincreditmemoID?>);
    ...
    function ajaxCall(passData) {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('creditmemo/savecreditmemoforedit')?>",
            data: passData,
            async: true,
            dataType: "html",
            success: function(data) {
    ...
    function errrorLoader(response) {
        if(response.status) {
            ...
            $('#editCreditMemoButton').removeAttr('disabled');
