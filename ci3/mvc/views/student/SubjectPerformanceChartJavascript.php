...
$(function() {
    ...
    function LoadSubjectPerformance()
    {
      ...
      const series = [
        <?php
              ...
              foreach ($mandatorysubjects as $mandatorysubject) {
                ...
                if(customCompute($markpercentages)) {
                    foreach ($markpercentages as $markpercentageID) {
                      ...
                      if(isset($marks[$studentID][$exam->examID][$mandatorysubject->subjectID][$markpercentageID]) && $f) {
                          if($marks[$studentID][$exam->examID][$mandatorysubject->subjectID][$markpercentageID] > 0) {
                              $data .= "['". $mandatorysubject->subjectID ."', ". $marks[$studentID][$exam->examID][$mandatorysubject->subjectID][$markpercentageID] ."],";
                          } else {
          ...
          foreach ($optionalsubjects as $optionalsubject) {
            $targetData .= "['". $optionalsubject->subjectID ."', ". $optionalsubject->targetmark ."],";
          }
          $targetData .= "]";
      ...
      $('#container').highcharts({
        subjects,
        title: {
          text: 'Student\'s subject performance in relation to subject target',
          align: 'left'
        },
        plotOptions: {
          series: {
