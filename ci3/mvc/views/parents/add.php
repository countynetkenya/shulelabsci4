            ...
            <div class="col-sm-10">
                ...
                <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
                    ...
                    ?>
                        ...
                        <div class="col-sm-2">
                          <select name="countrycode[]" class='countrycode form-control'></select>
                        </div>
                        <div class="col-sm-4">
                            <input type="number" class="form-control phone" name="phone[]" value="<?=set_value('phone')?>" placeholder="720000000">
                        </div>
                        <span class="col-sm-4 control-label">
                    ...
                    <input type="button" style="margin-top:3px; margin-bottom:10px" class="btn btn-primary btn-sm col-sm-offset-2 addPhoneBtn" value="Add" />
...
for (var i=0;i<countries.length;i++){
   if(countries[i].data.dial_code == "+254")
      option += '<option selected value="'+ countries[i].data.dial_code.replace(/^\+/, "") + '">' + countries[i].data.name + '</option>';
   else
      option += '<option value="'+ countries[i].data.dial_code.replace(/^\+/, "") + '">' + countries[i].data.name + '</option>';
}
...
function productItemDesign() {
    var text = '<div class="form-group">';
        text += '<div class="col-sm-2 col-sm-offset-2">';
        text += '<select name="countrycode[]" class="form-control countrycode">';
        for (var i=0;i<countries.length;i++){
           if(countries[i].data.dial_code == "+254")
              text += '<option selected value="'+ countries[i].data.dial_code.replace(/^\+/, "") + '">' + countries[i].data.name + '</option>';
           else
              text += '<option value="'+ countries[i].data.dial_code.replace(/^\+/, "") + '">' + countries[i].data.name + '</option>';
        }
        text += '</select>';
        text += '</div>';
        text += '<div class="col-sm-4">';
        text += '<input type="number" class="form-control" name="phone[]" value="" placeholder="720000000">';
        text += '</div>';
        text += '<span class="col-sm-4 control-label">';
        text += '</span>';
        text += '</div>';
...
$(document).on('click', '.addPhoneBtn', function() {
    var appendData  = productItemDesign();
    $(appendData).insertBefore( ".addPhoneBtn" );
    $('.countrycode').last().select2();
});
