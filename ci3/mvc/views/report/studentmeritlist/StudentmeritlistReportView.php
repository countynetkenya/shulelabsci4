<div class="box">
    <div class="box-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"> <?=$this->lang->line('menu_studentmeritlistreport')?></li>
        </ol>
    </div><!-- /.box-header -->
    <!-- form start -->
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                <div class="form-group col-sm-4" id="examDiv">
                    <label><?=$this->lang->line("studentmeritlistreport_exam")?><span class="text-red"> * </span></label>
                    <?php
                        $examArray = array(
                            "0" => $this->lang->line("studentmeritlistreport_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="classesDiv">
                    <label><?=$this->lang->line("studentmeritlistreport_class")?></label>
                    <?php
                        $classesArray = array(
                            "0" => $this->lang->line("studentmeritlistreport_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="sectionDiv">
                    <label><?=$this->lang->line("studentmeritlistreport_section")?></label>
                    <?php
                        $sectionArray = array(
                            "0" => $this->lang->line("studentmeritlistreport_please_select"),
                        );
                ...
                <div class="form-group col-sm-4" id="subjectDiv">
                    <label><?=$this->lang->line("studentmeritlistreport_subject")?></label>
                    <?php
                        $subjectArray = array(
                            "0" => $this->lang->line("studentmeritlistreport_please_select"),
                        );
                ...
                <div class="col-sm-4">
                    <button id="get_tabulationsheetreport" class="btn btn-success" style="margin-top:23px;"> <?=$this->lang->line("studentmeritlistreport_submit")?></button>
                </div>
            </div>
        </div><!-- row -->
    </div><!-- Body -->
...
<script type="text/javascript">
    ...
    $(document).on('change', "#classesID", function() {
        ...
        if(classesID == '0'){
            $('#sectionDiv').hide('slow');
            $('#subjectDiv').hide('slow');
            $('#sectionID').html('<option value="0">'+"<?=$this->lang->line("studentmeritlistreport_please_select")?>"+'</option>');
            $('#subjectID').html('<option value="0">'+"<?=$this->lang->line("studentmeritlistreport_please_select")?>"+'</option>');
        } else {
            $('#sectionDiv').show('slow');
            $('#subjectDiv').show('slow');
            $.ajax({
                type: 'POST',
                url: "<?=base_url('studentmeritlistreport/getSection')?>",
            ...
            $.ajax({
                type: 'POST',
                url: "<?=base_url('studentmeritlistreport/getSubject')?>",
                data: {"classesID" : classesID},
                dataType: "html",
                success: function(data) {
                   $('#subjectID').html(data);
    ...
    function ajaxCall(passData) {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('studentmeritlistreport/getStudentmeritlistReport')?>",
            data: passData,
            dataType: "html",
            success: function(data) {
