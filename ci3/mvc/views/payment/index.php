        ...
        <div class="box">
            ...
            <div class="box-body">
                <form role="form" method="post" enctype="multipart/form-data" id="invoiceDataForm">
					...
					<?php if (permissionChecker('payment_add') && $usertypeID != 4) {?>
					...
					<div class="form-group <?=form_error('schooltermID') ? 'has-error' : '' ?>" >
                        <label for="schooltermID">
                            <?=$this->lang->line("payment_schooltermID")?>
                        </label>
                            <?php
					...
					<?php }elseif ($usertypeID == 4) {?>
						<div class="phoneDiv form-group <?=form_error('phonenumber') ? 'has-error' : '' ?>" >
                        <label for="phone-number">
                            <?=$this->lang->line("global_phone_number")?> <span class="text-red">*</span>
                        </label>
                        <input type="text" class="form-control" id="phone-number" name="phonenumber" placeholder="25472xxxxxxx">
                        <span class="text-red">
          ...
          <?php if ($config['active'] == "1" && $usertypeID != 4) {
            if(empty($config['sessionAccessToken'])) {?>
              <div class="form-group">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered feetype-style" style="font-size: 16px;">
                        ...
                        <tbody id="studentList">
                						<?php if ($usertypeID == 4) { $i=1;
                						foreach ($students as $student) { $randID = floor(rand() * floor(9999999999999999));?>
                						<tr id="tr_<?=$randID?>" paymentstudentID="<?=$student->studentID?>">
                							<td><?=$i?></td>
                							<td><?=$student->name?></td>
                							<td><input type="text" class="form-control change-amount" id="td_amount_id_<?=$randID?>" data-amount-id="<?=$randID?>" value="<?=$student->balance > 0 ? $student->balance : ''?>"></td>
                							<td>
                								<select class="form-control payment-method select2" id="td_payment_method_id_<?=$randID?>" data-payment-method-id="<?=$randID?>" disabled>
                                  <?php foreach($paymentmethods as $paymentmethod) {?>
                                  <option value="<?=$paymentmethod->paymenttypesID?>" <?php echo (strtolower($paymentmethod->paymenttypes)=="m-pesa" || strtolower($paymentmethod->paymenttypes)=="mpesa") ? "selected" : ""?>><?=$paymentmethod->paymenttypes?></option>
                                  <?php }?>
                								</select>
                							</td>
                							<td>
                								<a style="margin-top:3px" href="#" class="btn btn-danger btn-sm deleteBtn" id="student_<?=$randID?>" data-student-id="<?=$randID?>"><i class="fa fa-trash-o"></i></a>
                							</td>
                						</tr>
...
<script type="text/javascript">
    ...
    var permission = <?= (permissionChecker('payment_add') != 1) ? 0 : 1; ?>;
    ...
    var text = '<tr id="tr_'+randID+'" paymentstudentID="'+studentID+'">';
		...
		text += '<td>';
			...
			for (var i = 0; i < paymentmethods.length; i++) {
				text += ('<option value="'+ paymentmethods[i].paymenttypesID +'">'+ paymentmethods[i].paymenttypes +'</option>');
			}
	...
	$(document).on('click', '#addPaymentButton', function() {
        ...
        var studentitems = $('tr[id^=tr_]').map(function(){
            return { studentID : $(this).attr('paymentstudentID'), amount: $(this).children().eq(2).children().val(), paymenttypeID: $(this).children().eq(3).find('select').val(), transactionID: $(this).children().eq(4).children().val()};
        }).get();
    ...
    function errrorLoader(response) {
        if(response.status && response.id) {
            window.location = "<?=base_url("paymenthistory/view/")?>" + response.id;
