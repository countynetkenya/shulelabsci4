    ...
    <div class="well">
        <div class="row">
            <div class="col-sm-6">
                ...
                <a href="<?=base_url("inventoryinvoice/print/". $productsale->productsaleID)?>" class="btn-cs btn-sm-cs" style="text-decoration: none;" role="button" target="_blank"><?=$this->lang->line('print')?></a>
        ...
        <section class="content invoice" >
            ...
            <div class="row invoice-info">
                <div class="col-sm-4 invoice-col" style="font-size: 16px;">
                    <?php  echo $this->lang->line("inventoryinvoice_from"); ?>
                    <?php if(customCompute($siteinfos)) { ?>
                        <address>
                            <strong><?=$siteinfos->sname?></strong><br>
                            <?=$siteinfos->address?><br>
                            <?=$this->lang->line("inventoryinvoice_phone"). " : ". $siteinfos->phone?><br>
                            <?=$this->lang->line("inventoryinvoice_email"). " : ". $siteinfos->email?><br>
                        </address>
                    <?php } ?>
                </div>
                <div class="col-sm-4 invoice-col" style="font-size: 16px;">
                    <?php  echo $this->lang->line("inventoryinvoice_to"); ?>
                    <address>
                        <?php if(customCompute($user)) { ?>
                            <?php if(isset($user->name)) { ?>
                                <strong><?=$user->name?></strong><br>
                                <?=$this->lang->line("inventoryinvoice_role")?> : <?=isset($usertypes[$user->usertypeID]) ? $usertypes[$user->usertypeID] : 'School'?><br>
                                <?=($user->address == NULL) ? '' : $user->address.'<br>' ?>
                                <?=$this->lang->line("inventoryinvoice_phone"). " : ". $user->phone?><br>
                                <?=$this->lang->line("inventoryinvoice_email"). " : ". $user->email?><br>
                            <?php } else { ?>
                                <strong><?=$user->srname?></strong><br>
                                <?=$this->lang->line("inventoryinvoice_role")?> : <?=isset($usertypes[3]) ? $usertypes[3] : ''?><br>
                                <?=($user->address == NULL) ? '' : $user->address.'<br>' ?>
                                <?=$this->lang->line("inventoryinvoice_phone"). " : "?><br>
                                <?=$this->lang->line("inventoryinvoice_email"). " : "?><br>
                            <?php } ?>
                        <?php } ?>
                    </address>
                </div>
                <div class="col-sm-4 invoice-col" style="font-size: 16px;">
                    <b><?=$this->lang->line("inventoryinvoice_referenceno")." : ".$productsale->productsalereferenceno?></b>
                    <br>
                    <?php
                        ...
                        <table class="table table-bordered product-style">
                            <thead>
                                <tr>
                                    ...
                                    <th class="col-lg-4"><?=$this->lang->line('inventoryinvoice_description')?></th>
                                    <th class="col-lg-2"><?=$this->lang->line('inventoryinvoice_unit_price')?></th>
                                    <th class="col-lg-2"><?=$this->lang->line('inventoryinvoice_quantity')?></th>
                                    <th class="col-lg-2"><?=$this->lang->line('inventoryinvoice_subtotal')?></th>
                                </tr>
                            </thead>
                            <tbody>
