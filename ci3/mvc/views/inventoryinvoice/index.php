        ...
        <div class="row">
            <div class="col-sm-12">
                <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                    <?php if(permissionChecker('inventoryinvoice_add')) { ?>
                        <h5 class="page-header">
                            <a href="<?php echo base_url('inventoryinvoice/add') ?>">
                    ...
                    <table id="example2" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
                                ...
                                <th><?=$this->lang->line('inventoryinvoice_referenceno')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_role')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_user')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_date')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_file')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_grandtotal')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_paid')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_balance')?></th>
                                <?php if(permissionChecker('inventoryinvoice_edit') || permissionChecker('inventoryinvoice_delete') || permissionChecker('inventoryinvoice_view')) { ?>
                        ...
                        <tbody>
                            <?php if(customCompute($productsales)) {$i = 1; foreach($productsales as $productsale) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('inventoryinvoice_referenceno')?>">
                                        <?=$productsale->productsalereferenceno;?>
                                        <?=($productsale->productsalerefund) ? '<span class="text-red">('. $this->lang->line('inventoryinvoice_refund') .')</span>' : ''?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('inventoryinvoice_role')?>">
                                        <?=isset($usertypes[$productsale->productsalecustomertypeID]) ? $usertypes[$productsale->productsalecustomertypeID] : 'School'?>
                                    </td>

                                    <td data-title="<?=$this->lang->line('inventoryinvoice_user')?>">
                                        <?php
                                            ...
                                            } elseif($productsale->productsalecustomertypeID == "school") {
                                                echo $schools[$productsale->productsalecustomerID];
                                            }
                                        ?>
                                    </td>

                                    <td data-title="<?=$this->lang->line('inventoryinvoice_date')?>">
                                        <?=date('d M Y', strtotime($productsale->productsaledate));?>
                                    </td>

                                    <td data-title="<?=$this->lang->line('inventoryinvoice_file')?>">
                                        <?php
                                            if($productsale->productsalefileorginalname) { echo btn_download_file('inventoryinvoice/download/'.$productsale->productsaleID, namesorting($productsale->productsalefileorginalname, 12), $this->lang->line('download'));
                                            }
                                        ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('inventoryinvoice_grandtotal')?>">
                                    ...
                                    <td data-title="<?=$this->lang->line('inventoryinvoice_paid')?>">
                                    ...
                                    <td data-title="<?=$this->lang->line('inventoryinvoice_balance')?>">
                                        <?php
                                    ...
                                    <?php if(permissionChecker('inventoryinvoice_edit') || permissionChecker('inventoryinvoice_delete') || permissionChecker('inventoryinvoice_view')) { ?>
                                        <td data-title="<?=$this->lang->line('action')?>">
                                            <?php
                                                echo btn_view('inventoryinvoice/view/'.$productsale->productsaleID, $this->lang->line('view'));
                                                ...
                                                if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
                                                    if(isset($grandtotalandpaid['totalpaid'][$productsale->productsaleID]) && $grandtotalandpaid['totalpaid'][$productsale->productsaleID] > 0) {
                                                        if($productsale->productsalerefund == 0) {
                                                            if(permissionChecker('inventoryinvoice_edit') && permissionChecker('inventoryinvoice_delete')) {
                                                                echo btn_cancel('inventoryinvoice/cancel/'.$productsale->productsaleID, $this->lang->line('cancel'));
                                                            }
                                                        }
                                                    } else {
                                                        echo btn_edit('inventoryinvoice/edit/'.$productsale->productsaleID, $this->lang->line('edit'));
                                                        echo btn_delete('inventoryinvoice/delete/'.$productsale->productsaleID, $this->lang->line('delete'));
                                                    }
                                                }

                                                if(permissionChecker('inventoryinvoice_view')) {
                                                    echo '<a href="#paymentlist" id="'.$productsale->productsaleID.'" class="btn btn-info btn-xs mrg getpaymentinfobtn" rel="tooltip" data-toggle="modal"><i class="fa fa-list-ul" data-toggle="tooltip" data-placement="top" data-original-title="'.$this->lang->line('inventoryinvoice_view_payments').'"></i></a>';
                                                }
                                            ?>
                                        </td>
                                    <?php } ?>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?=$this->lang->line('inventoryinvoice_add_payment')?></h4>
            </div>
                    ...
                    <div class="col-sm-6">
                        <div class="col-sm-12">
                            <div class="form-group" id="productsalepaiddateerrorDiv">
                                <label for="productsalepaiddate"><?=$this->lang->line('inventoryinvoice_date')?> <span class="text-red">*</span></label>
                    ...
                    <div class="col-sm-6">
                        <div class="col-sm-12">
                            <div class="form-group" id="productsalepaidreferencenoerrorDiv">
                                <label for="productsalepaidreferenceno"><?=$this->lang->line('inventoryinvoice_referenceno')?> <span class="text-red">*</span></label>
                    ...
                    <div class="col-sm-6">
                        <div class="col-sm-12">
                            <div class="form-group" id="productsalepaidamounterrorDiv">
                                <label for="productsalepaidamount"><?=$this->lang->line('inventoryinvoice_amount')?> <span class="text-red">*</span></label>
                    ...
                    <div class="col-sm-6">
                        <div class="col-sm-12">
                            <div class="form-group" id="productsalepaidpaymentmethoderrorDiv">
                                <label for="productsalepaidpaymentmethod"><?=$this->lang->line('inventoryinvoice_paymentmethod')?> <span class="text-red">*</span></label>
                                <?php
                                    $paymentmethodArray = array(0 => $this->lang->line('inventoryinvoice_select_paymentmethod'));
                                    foreach($paymentmethods as $paymentmethod) {
                                      $paymentmethodArray[$paymentmethod->paymenttypesID] =  $paymentmethod->paymenttypes;
                                    }
                    ...
                    <div class="col-sm-6">
                        <div class="col-sm-12">
                            <div class="form-group" id="productsalepaidfileerrorDiv">
                                <label for="productsalepaidfile"><?=$this->lang->line('inventoryinvoice_file')?></label>
            ...
            <div class="modal-footer">
                ...
                <input type="button" id="add_payment_button" class="btn btn-success" value="<?=$this->lang->line("inventoryinvoice_add_payment")?>" />
            </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="paymentlist">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                <h4 class="modal-title"><?=$this->lang->line('inventoryinvoice_view_payments')?></h4>
            </div>
            <div class="modal-body">
                <div id="hide-table">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th><?=$this->lang->line('inventoryinvoice_date')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_referenceno')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_amount')?></th>
                                <th><?=$this->lang->line('inventoryinvoice_paid_by')?></th>
                                <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
...
<script type="text/javascript">
    ...
    $('.getsaleinfobtn').click(function() {
        ...
        if(productsaleID > 0) {
            $.ajax({
                type: 'POST',
                url: "<?=base_url('inventoryinvoice/getsaleinfo')?>",
    ...
    $('.getpaymentinfobtn').click(function() {
        ...
        if(productsaleID > 0) {
            $.ajax({
                type: 'POST',
                url: "<?=base_url('inventoryinvoice/paymentlist')?>",
                data: {'productsaleID' : productsaleID},
                dataType: "html",
                success: function(data) {
    ...
    function ajaxCall(passData) {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('inventoryinvoice/saveproductsalepayment')?>",
            data: passData,
            async: true,
            dataType: "html",
            success: function(data) {
