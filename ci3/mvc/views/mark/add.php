                ...
                <?php if(customCompute($students)) { ?>
                        ...
                        <table id="example2" class="table table-striped table-bordered table-hover dataTable no-footer">
                            <thead>
                                <tr>
                                    ...
                                    <th><?=$this->lang->line('mark_teacher_comment')?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if(customCompute($students)) {$i = 1; foreach($students as $student) { foreach ($marks as $mark) { if($student->studentID == $mark->studentID) {   ?>
                                    <tr>
                                        ...
                                        ?>
                                        <td data-title="<?=$this->lang->line('mark_teacher_comment')?>">
                                            <textarea class="form-control teacher_comment" name="comment-<?=$mark->markID?>"><?php echo $mark->teacher_comment;?></textarea>
                                        </td>
                                    </tr>
                                <?php $i++;  }}}} ?>
                            </tbody>
                    ...
                    <script type="text/javascript">
                        ...
                        $("#add_mark").click(function() {
                            ...
                            var comment = $('textarea[name^=comment]').map(function(){
                                return { comment: this.name , value: this.value};
                            }).get();

                            $.ajax({
                                type: 'POST',
                                url: "<?=base_url('mark/mark_send')?>",
                                data: {"examID" : "<?=$set_exam?>", "classesID" : "<?=$set_classes?>", "subjectID" : "<?=$set_subject?>", "comments" : comment, "inputs" : mark},
                                dataType: "html",
