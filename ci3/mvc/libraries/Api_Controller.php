...
class Api_Controller extends REST_Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        $this->data["siteinfos"]            = $this->site_m->get_site($this->session->userdata('schoolID'));
        $schoolyearID                       = $this->data['siteinfos']->school_year;
        $this->data['schoolyearobj']        = $this->schoolyear_m->get_obj_schoolyear(array('schoolyearID' => $schoolyearID));
    ...
    private function userInfo( $array )
    {
        ...
        if(customCompute($userFoundInfo)) {
            $schoolID     = $userFoundInfo->selected_schoolID;
            ...
            if($this->session->userdata('usertypeID') == 1 && $this->session->userdata('loginuserID') == 1) {
                ...
                if(isset($session['schoolID'])) {
                    $features = $this->permission_m->get_modules_with_permission(array('id' => $session['usertypeID'], 'schoolID' => $session['schoolID']));
    ...
    private function permissionControl()
    {
        if(!empty($this->uri->segment(3))) {
            ...
            if(!empty($mode)) {
                ...
                if($mode!="school" && isset($permissionSet[$mode]) && ($permissionSet[$mode] == 'no')) {
                    $this->response([
                        'status'  => false,
