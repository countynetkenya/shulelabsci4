...
class OrchestrationController extends BaseController
{
    ...
    public function __construct() { $this->service = new OrchestrationService(); }
    ...
    public function store() {
        ...
        if ($this->service->create($data)) return redirect()->to('/orchestration')->with('success', 'Workflow created successfully');
    ...
    public function edit($id) {
        ...
        if (!$workflow) return redirect()->to('/orchestration')->with('error', 'Workflow not found');
    ...
    public function update($id) {
        ...
        if ($this->service->update($id, $data)) return redirect()->to('/orchestration')->with('success', 'Workflow updated successfully');
        return redirect()->back()->withInput()->with('error', 'Failed to update workflow');
    }
    public function delete($id) {
        if (!session()->get('is_logged_in')) return redirect()->to('/login');
        if ($this->service->delete($id)) return redirect()->to('/orchestration')->with('success', 'Workflow deleted successfully');
