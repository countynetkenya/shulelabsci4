...
## 🎓 LMS Module Development Protocol

The Learning Management System (LMS) is a core module with specific architectural requirements:

### 1. Architecture (The "Hostel Method")
- **Namespace:** `Modules\Learning`
- **Separation:**
  - `Controllers\Web`: HTML views for Teachers/Admins (Course creation, Lesson management).
  - `Controllers\Api`: JSON endpoints for Mobile App (Student enrollment, Progress tracking).
- **Service Layer:** `LearningService` is the SINGLE entry point for all logic. It aggregates `CourseModel`, `LessonModel`, `EnrollmentModel`, and `ProgressModel`.

### 2. Mobile-First API Contract
The API is the primary consumer for Students. Ensure these endpoints remain stable:
- `GET /api/learning/courses`: List available courses.
- `GET /api/learning/courses/{id}`: Course details + Lesson list.
- `POST /api/learning/enrollments`: Join a course.
- `POST /api/learning/progress`: Mark lesson complete.

### 3. Testing Strategy
- **Feature Tests Only:** Do not rely on legacy `tests/Learning/LearningServiceTest.php`.
- **New Tests:** Use `tests/Learning/CoursesTest.php`, `LessonsTest.php`, `EnrollmentsApiTest.php`.
- **JSON Assertions:** Use `json_decode` or `assertSee` for API tests to avoid structure mismatch flakiness.
